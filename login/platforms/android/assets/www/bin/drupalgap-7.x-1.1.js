...
function io_armar_galeria(table_css_selector, rows) {
	  try {
	    ...
	    for (var i = 0; i < rows.length; i++) {
	      ...
	      $('<div class="contact" style="position:relative; float: left; margin-left:5px; margin-right:5px;">').html(div_contacto).appendTo($(table_css_selector));
	    }

	    $(table_css_selector).rebuild();
	  }
	  catch (error) { console.log('io_armar_galeria - ' + error); }
	}
...
function io_drupalgap_table_populate(table_css_selector, rows) {
	  try {
	    ...
	    for (var i = 0; i < rows.length; i=i+2) {
	      ...
	      var rowhtml = '';
	      for (var j = 0; j < row.length; j++) {
	          rowhtml = rowhtml + '<td style="text-align: center;">' + row[j] + '</td>';
	          rowhtml = rowhtml + '<td style="text-align: center;">' + row2[j] + '</td>';
	      }
	      $('<tr>').html(rowhtml).appendTo($(table_css_selector));

	      /*if(i + 2 >= rows.length){
	        alert("i + 2 > rows.length TRUE");
	        rowhtml = rowhtml + '<td style="text-align: center;">' + row[i+1] + '</td>';
	        break;
	      }*/
	      if(i + 3 == rows.length){
	        var row = rows[i+2];
	        var rowhtml = '';
	        rowhtml = rowhtml + '<td style="text-align: center;">' + row[0] + '</td>';
	        $('<tr>').html(rowhtml).appendTo($(table_css_selector));
	        break;
	      }
    ...
    switch (delta) {
      ...
      case 'powered_by':
          return '<p style="text-align: center;">' + t('Realizado por') + ': ' +
            l('ideasOrange', 'http://www.ideasorange.com', {InAppBrowser: true}) +
          '</p>';
          break;
      case 'help':
...
// Defaults to sessionStorage for storing the Facebook token
    ...
    openFB.init({appId: '474315632721064', tokenStore: window.localStorage});
    function login() {
        openFB.login(
                function(response) {
    ...
    // IO: Funcion que loguea al usuario en la app y si no lo encuentra como usuario de 
    ...
    function getInfoForLogin() {
        openFB.api({
            ...
            success: function(data) {
                ...
                error:function(xhr, status, message){
                	//alert(message);
                	//Se crea un nuevo usuario a traves de los datos de Facebook
                	var account = {
                			name:data.name,
                			mail:data.email,
                			pass:data.id
                			//imagen:'http://graph.facebook.com/' + data.id + '/picture'
                	};
                	user_save(account, {
                		success:function(result) {
                			//drupalgap_toast('<p>Ya sos parte de La Previa Paraná !</p>');
