...
function drupalgap_init() {
	var dg = {
		...
		locale : {}, /*
						 * holds onto language json objects, keyed by language
						 * code
						 */
		messages : [],
		menus : {},
		menu_links : {},
		menu_router : {}, /*
							 * @todo - doesn't appear to be used at all, remove
							 * it
							 */
		mvc : {
			models : {},
		...
		page : {
			...
			process : true,
			options : {}
		/* holds the current page's options, eg. reloadPage, etc. */
		},
...
function drupalgap_onload() {
	try {
		...
		// At this point, the Drupal object has been initialized by jDrupal and
		// the
		// app/settings.js file was loaded in <head>. Let's add DrupalGap's
		// modules
		// onto the Drupal JSON object. Remember, all of the module source code
		// is
		// included via the makefile's bin generation. However, the core modules
		// hook_install() implementations haven't been called yet, so we add
		// them to
		// the module listing so they can be invoked later on.
		var modules = [ 'drupalgap', 'block', 'comment', 'contact', 'entity',
				'field', 'file', 'image', 'menu', 'mvc', 'node', 'search',
				'system', 'taxonomy', 'user', 'views' ];
		...
		// otherwise specified by the settings.js file. If it is for phonegap,
		// we'll
		...
		case 'phonegap':
			document.addEventListener('deviceready', _drupalgap_deviceready,
					false);
			break;
		case 'web-app':
			...
			break;
		default:
			console.log('drupalgap_onload - unknown mode ('
					+ drupalgap.settings.mode + ')');
			return;
			break;
		}
...
function _drupalgap_deviceready() {
	try {
		...
		// Check device connection. If the device is offline, warn the user and
		// then
		...
		if (!drupalgap.online) {
			...
			// Device is online, let's call any implementations of
			// hook_deviceready().
			// If any implementation returns false, that means they don't want
			// DrupalGap to continue with the System Connect call, so we'll skip
			// that
			...
			if (!proceed) {
				// @todo - if module's are going to skip the System Connect
				// call, then
				// we need to make sure Drupal.user is set up with appropriate
				// defaults.
			} else {
	...
	try {
		...
		return {
			...
			error : function(jqXHR, textStatus, errorThrown) {
				...
				msg += ' - '
						+ t('Check your device\'s connection and check that')
...
function drupalgap_bootstrap() {
	try {
		...
		// already been loaded at this point), load the theme and all blocks.
		// Then
...
function drupalgap_load_modules() {
	try {
		...
		// We only need to load contrib and custom modules because core modules
		// are
		...
		$
				.each(
						...
						function(index, bundle) {
							// Let's be nice and try to load any old
							// drupalgap.modules declarations
							// in developers settings.js files for a while, but
							// throw a warning to
							// encourage them to update. This code can be
							// removed after a few
							// releases to help developers get caught up without
							// angering them.
							if (drupalgap.modules && drupalgap.modules[bundle]
							...
							$
									.each(
											...
											function(module_name, module) {
												// If the module object is
												// empty, initialize a module
												// object.
												if ($.isEmptyObject(module)) {
													Drupal.modules[bundle][module_name] = module_object_template(module_name);
													module = Drupal.modules[bundle][module_name];
												}
												// If the module's name isn't
												// set, set it.
												if (!module.name) {
												...
												module_base_path = dir + '/'
														+ module.name;
												// Add module .js file to array
												// of paths to load.
												...
												// with this module, add them to
												// the
												// list of paths to include.
												if (module.includes != null
														&& module.includes.length != 0) {
													$
															.each(
																	module.includes,
																	function(
																			...
																			include_object) {
																		modules_paths
																				.push(module_base_path
																						+ '/'
																						+ include_object.name
																						+ '.js');
																	});
												}
												// Now load all the paths for
												// this module.
												$
														.each(
																modules_paths,
	...
	try {
		if (!drupalgap.settings.theme) {
			...
			if (theme_name != 'easystreet3' && theme_name != 'ava') {
				theme_path = 'app/themes/' + theme_name + '/' + theme_name
						+ '.js';
			}
			if (!drupalgap_file_exists(theme_path)) {
				var error_msg = 'drupalgap_theme_load - '
						+ t('Failed to load theme!') + ' '
						+ t('The theme\'s JS file does not exist') + ': '
						+ theme_path;
			...
			if (drupalgap_function_exists(template_info_function)) {
				...
				drupalgap.theme = fn();
				// For each region in the name, set the 'name' value on the
				// region JSON.
				...
				for ( var i = 0; i < regions.length; i++) {
					...
					if (typeof drupalgap.theme.regions[region] === 'undefined') {
						console.log('WARNING: drupalgap_load_theme() - The "'
								+ theme_name + '" theme does not have the "'
								+ region + '" region specified in "'
								+ theme_name + '_info()."');
					}
				}
				// Theme loaded successfully! Set the drupalgap.theme_path and
				// return
				// true.
				drupalgap.theme_path = theme_path.replace('/' + theme_name
						+ '.js', '');
				return true;
			} else {
				var error_msg = 'drupalgap_load_theme() - ' + t('failed')
						+ ' - ' + template_info_function + '() '
						+ t('does not exist!');
				drupalgap_alert(error_msg);
			}
		}
...
/**
 * Given a path to a javascript file relative to the app's www directory, this
 * will load the javascript file so it will be available in scope.
 */
	...
	try {
		...
		jQuery.ajax({
			...
			error : function(xhr, textStatus, errorThrown) {
				console.log('drupalgap_add_js - error - (' + data + ' : '
 ...
 * the alert is displayed to the user. You may optionally pass in a second
 * argument as a JSON object with the following properties: alertCallback - the
 * function to call after the user presses OK title - the title to use on the
 * alert box, defaults to 'Alert' buttonName - the text to place on the button,
 * default to 'OK'
 * 
 * @param {String}
 *            message
 ...
 * execute while the confirmation is displayed to the user. You may optionally
 * pass in a second argument as a JSON object with the following properties:
 * confirmCallback - the function to call after the user presses a button, the
 * button's label is passed to this function. title - the title to use on the
 * alert box, defaults to 'Confirm' buttonLabels - the text to place on the OK,
 * and Cancel buttons, separated by comma.
 * 
 * @param {String}
 *            message
 * @return {Boolean}
...
function drupalgap_confirm(message) {
	try {
		...
		// The phonegap confirm dialog doesn't seem to work in Ripple, so just
		// use
...
/**
 * Show a non intrusive alert message. You may optionally pass in an integer
 * value as the second argument to specify how many milliseconds to wait before
 * closing the message. Likewise, you can pass in a third argument to specify
 * how long to wait before opening the message.
 * 
 * @param {string}
	...
	try {
		...
		for ( var i = 0; i < modules.length; i++) {
			...
			for ( var j = 0; j < languages.length; j++) {
				...
				var file_path = drupalgap_get_path('module', module)
						+ '/locale/' + language_code + '.json';
				var translations = drupalgap_file_get_contents(file_path, {
					dataType : 'json'
				});
				if (typeof drupalgap.locale[language_code] === 'undefined') {
					drupalgap.locale[language_code] = translations;
				} else {
					$.extend(drupalgap.locale[language_code],
							drupalgap.locale[language_code], translations);
				}
			}
		}
...
/**
 * Checks the devices connection and sets drupalgap.online to true if the device
 * has a connection, false otherwise.
 * 
 * @return {String} A string indicating the type of connection according to
 *         PhoneGap.
 */
function drupalgap_check_connection() {
	try {

		// If we're not in PhoneGap (i.e. a web app environment, or Ripple),
		// we'll
		// assume we have a connection. Is this a terrible assumption? Anybody
		// out
		// there know?
...
/**
 * @deprecated Use empty() instead. Returns true if given value is empty. A
 *             generic way to test for emptiness.
 * @param {*}
 *            value
 * @return {Boolean}
 */
function drupalgap_empty(value) {
	try {
		console.log('WARNING: drupalgap_empty() is deprecated! '
				+ 'Use empty() instead.');
		return empty(value);
	} catch (error) {
...
/**
 * Reads entire file into a string and returns the string. Returns false if it
 * fails.
 * 
 * @param {String}
 *            path
...
function drupalgap_file_get_contents(path, options) {
	try {
		...
		var default_options = {
			...
			error : function(xhr, textStatus, errorThrown) {
				console
						.log('drupalgap_file_get_contents - failed to load file ('
								+ path + ')');
			}
		};
	...
	try {
		...
		$.each(units, function(key, value) {
			...
			if (interval >= value) {
				...
				output += (output ? ' ' : '')
						+ drupalgap_format_plural(count, key[0], key[1]);
...
function drupalgap_image_path(uri) {
	try {
		...
		if (!altered) {
			...
			var src = Drupal.settings.site_path + Drupal.settings.base_path
					+ uri;
 ...
 *             makefile. Loads the js files in includes specified by
 *             drupalgap.includes.
 */
...
function io_armar_galeria(table_css_selector, rows) {
	try {
		...
		for ( var i = 0; i < rows.length; i++) {
			...
			var div_contacto = rows[i];

			$(
					'<div class="contact" style="position:relative; float: left; margin: 0 auto; width:50%; height:25%;">')
					.html(div_contacto).appendTo($(table_css_selector));
		}

		$(table_css_selector).rebuild();
	} catch (error) {
		console.log('io_armar_galeria - ' + error);
	}
}
...
function io_drupalgap_table_populate(table_css_selector, rows) {
	try {
		...
		for ( var i = 0; i < rows.length; i = i + 2) {
			...
			var rowhtml = '';
			for ( var j = 0; j < row.length; j++) {
				rowhtml = rowhtml + '<td style="text-align: center;">' + row[j]
						+ '</td>';
				rowhtml = rowhtml + '<td style="text-align: center;">'
						+ row2[j] + '</td>';
			}
			$('<tr>').html(rowhtml).appendTo($(table_css_selector));

			/*
			 * if(i + 2 >= rows.length){ alert("i + 2 > rows.length TRUE");
			 * rowhtml = rowhtml + '<td style="text-align: center;">' +
			 * row[i+1] + '</td>'; break; }
			 */
			if (i + 3 == rows.length) {
				var row = rows[i + 2];
				var rowhtml = '';
				rowhtml = rowhtml + '<td style="text-align: center;">' + row[0]
						+ '</td>';
				$('<tr>').html(rowhtml).appendTo($(table_css_selector));
				break;
			}
...
function drupalgap_jqm_page_event_fire(event, callback, page_arguments) {
	try {
		// Concatenate the event name and the callback name together into a
		// unique
		...
		if ($.inArray(key, drupalgap.page.jqm_events) == -1
			...
			if (page_arguments) {
				// If the page arguments aren't an array, place them into an
				// array so
...
function drupalgap_jqm_page_events() {
	return [ 'pagebeforechange', 'pagebeforecreate', 'pagebeforehide',
			'pagebeforeload', 'pagebeforeshow', 'pagechange',
			'pagechangefailed', 'pagecreate', 'pagehide', 'pageinit',
			'pageload', 'pageloadfailed', 'pageremove', 'pageshow' ];
}
 ...
 * event. You may optionally pass in a unique second argument (string) to allow
 * the same page event handler to be fired more than once on a page.
 * 
 * @param {Object}
 *            options
 * @return {String}
...
function drupalgap_jqm_page_event_script_code(options) {
	try {
		...
		var script_code = '<script type="text/javascript">' + '$("#'
				+ options.page_id + '").on("' + options.jqm_page_event
				+ '", drupalgap_jqm_page_event_fire(' + event_fire_args + '));'
				+ '</script>';
		return script_code;
	} catch (error) {
...
function drupalgap_loading_message_hide() {
	try {
		/*
		 * $.mobile.loading('hide'); drupalgap.loading = false; drupalgap.loader =
		 * 'loading';
		 */
		setTimeout(function() {
			$.mobile.loading('hide');
...
function drupalgap_menu_access(path) {
	try {
		...
		if (drupalgap.menu_links[path]) {
			// Check to see if there is an access callback specified with the
			// menu
			// link.
			if (typeof drupalgap.menu_links[path].access_callback === 'undefined') {
				// No access call back specified, if there are any access
				// arguments
				// on the menu link, then it is assumed they are user permission
				// machine
				// names, so check that user account's role(s) for that
				// permission to
				// grant access.
				if (drupalgap.menu_links[path].access_arguments) {
					if ($.isArray(drupalgap.menu_links[path].access_arguments)) {
						$.each(drupalgap.menu_links[path].access_arguments,
					...
					// There is no access callback and no access arguments
					// specified with
				...
				if (drupalgap_function_exists(function_name)) {
					// Grab the access callback function. If there are any
					// access args
					...
					if (drupalgap.menu_links[path].access_arguments) {
						var access_arguments = drupalgap.menu_links[path].access_arguments
								.slice(0);
						// If we have an entity loaded, replace the first
						// integer we find
						// in the page arguments with the loaded entity.
						if (arguments[2]) {
							var entity = arguments[2];
							$.each(access_arguments, function(index,
									page_argument) {
						...
						return fn.apply(null, Array.prototype.slice
								.call(access_arguments));
					} else {
						return fn();
					}
				} else {
			...
			console.log('drupalgap_menu_access - path (' + path
					+ ') does not exist');
		}
		return access;
	} catch (error) {
...
function drupalgap_services_request_pre_postprocess_alter(options, result) {
	try {
		...
		if (options.service == 'system' && options.resource == 'connect') {
			...
			drupalgap.taxonomy_vocabularies = drupalgap_taxonomy_vocabularies_extract(result.taxonomy_vocabularies);
		...
		// Whenever a user logs in, out or registers, remove all pages from the
		// DOM.
		else if (options.service == 'user'
				&& (options.resource == 'logout' || options.resource == 'login' || options.resource == 'register')) {
		...
		// rebuilt properly next time they are loaded.
		else if (in_array(options.resource, [ 'create', 'update', 'delete' ])
			...
			if (options.resource != 'create') {
				...
				if (in_array(entity_type, [ 'taxonomy_vocabulary',
						'taxonomy_term' ])) {
					prefix = prefix.replace('_', '/', prefix);
				}
				paths.push(prefix + '/' + entity_id);
				paths.push(prefix + '/' + entity_id + '/view');
				// @todo This page won't get removed since it is the current
				// page...
				// maybe when an entity is updated or deleted, we need a
				// transitional
				// page that says "Deleting [entity]..." that way we can remove
				// this
				// page (since it is not possible to remove the current page in
				// jQM).
				// Actually now that I think about, we should have a
				// confirmation page
				// when deleting an entity just like Drupal, and that should
				// take care
				// of the deletion case. Not sure what to do about the update
				// case...
				// maybe some type of pageshow handler on entity view pages that
				// can
		...
		console.log('drupalgap_services_request_pre_postprocess_alter - '
				+ error);
	}
}
	...
	try {
		...
		$.each(modules, function(index, module) {
			...
			$.each(hook_theme, function(element, variables) {
				variables.path = drupalgap_get_path('theme',
...
/**
 * Returns an array with the parts for the current time. You may optionally pass
 * in a JS date object to use that date instead.
 * 
 * @return {Array}
 */
...
function theme_autocomplete(variables) {
	try {
		...
		// provided or generate a random one. Then finally attach the
		// autocomplete
		...
		if (variables.remote) {
			...
			js += '<script type="text/javascript">' + '$("#' + list_id
		...
		var selector = '#' + drupalgap_get_page_id() + ' #' + id
				+ ' + form.ui-filterable' + ' input[data-type="search"]';
		js += '<script type="text/javascript">'
				+ '_theme_autocomplete_input_selector["' + autocomplete_id
				+ '"] = \'' + selector + '\';' + '</script>';

		// If there was a default value, set it's key title in the
		// autocomplete's
		// text field.
 ...
 * 
 * @param {Object}
 *            list The unordered list that displays the items.
 * @param {Object}
 *            e
 * @param {Object}
 *            data
...
function _theme_autocomplete(list, e, data, autocomplete_id) {
	try {
		...
		if (typeof autocomplete.filter === 'undefined') {
			console.log('_theme_autocomplete - A "filter" was not supplied.');
			return;
		}

		// Make sure a value and/or label has been supplied so we know how to
		// render
		// the items in the autocomplete list.
		var value_provided = typeof autocomplete.value !== 'undefined' ? true
				: false;
		var label_provided = typeof autocomplete.label !== 'undefined' ? true
				: false;
		if (!value_provided && !label_provided) {
			console
					.log('_theme_autocomplete - A "value" and/or "label" was not supplied.');
			return;
		} else {
			// We have a value and/or label. If one isn't provided, set it equal
			// to
			// the other.
			if (!value_provided) {
		...
		var $ul = $(list), $input = $(data.input), value = $input.val(), html = '';
		// Clear the list.
		$ul.html('');
		...
		if (value && value.length > 0) {
			...
			// Let's first build the success handler that will place the items
			// into
			...
			_theme_autocomplete_success_handlers[autocomplete_id] = function(
				...
				try {
					...
					// If there are no results, and then if an empty callback
					// handler was
					// provided, call it.
					...
					if (result_items.length == 0) {
						...
						// Convert the result into an items array for a list.
						// Each item will
						// be a JSON object with a "value" and "label"
						// properties.
						var items = [];
						...
						// Now render the items, add them to list and refresh
						// the list.
						if (items.length != 0) {
					...
					// Anybody want to act on the completion of the
					// autocomplete?
					...
					console.log('_theme_autocomplete_success_handlers['
							+ _autocomplete_id + '] - ' + error);
				}
			};

			// Depending on the handler, build the path and call the Drupal site
			// for
			...
			case 'og':
				...
				// Retrieve JSON results. Keep in mind, we use this for
				// retrieving
				// Views JSON results and custom hook_menu() path results in
				// Drupal.
				views_datasource_get_view_result(
						path,
						{
							success : function(results) {
								// If this was a custom path, don't use a
								// wrapper around the
								// results like the one used by Views
								// Datasource.
								var wrapped = true;
								if (autocomplete.custom) {
									wrapped = false;
								...
								// Extract the result items based on the
								...
								fn(autocomplete_id, result_items, wrapped,
										results.view.child);
							}
						});
				break;
			...
			case 'base':
				var field_settings = autocomplete.field_info_field.settings;
				...
				$.each(field_settings.handler_settings.target_bundles,
			...
			case 'index':
				if (!autocomplete.entity_type) {
					console
							.log('WARNING - _theme_autocomplete - no entity_type provided');
					return;
				}
			...
			default:
				...
				// If we made it this far, and don't have a handler, then warn
				// the
				// developer.
				if (!handler) {
					console
							.log('WARNING - _theme_autocomplete - no handler provided');
					return;
				}
				...
				var options = {
					attributes : {
						...
						onclick : '_theme_autocomplete_click(\''
								+ variables.attributes.id + '\', this, \''
								+ variables.autocomplete_id + '\')'
					}
				};
 ...
 * 
 * @param {String}
 *            id The id of the hidden input that holds the value.
 * @param {Object}
 *            item The list item anchor that was just clicked.
 * @param {String}
 *            autocomplete_id
 */
function _theme_autocomplete_click(id, item, autocomplete_id) {
	try {
		// Set the hidden input with the value, and the text field with the
		// text.
		var list_id = id + '-list';
		$('#' + id).val($(item).attr('value'));
		$(_theme_autocomplete_input_selector[autocomplete_id]).val(
				$(item).html());
		...
		if (_theme_autocomplete_variables[autocomplete_id].item_onclick
...
function _theme_autocomplete_set_default_value_label(options) {
	try {
		setTimeout(function() {
			$(options.selector).val(options.default_value_label).trigger(
					'create');
		}, 250);
	} catch (error) {
...
/**
 * Given a block delta, this will return the corresponding block from
 * drupalgap.blocks.
 * 
 * @param {String}
 *            delta
...
function drupalgap_block_load(delta) {
	try {
		...
		if (block == null) {
			var msg = 'drupalgap_block_load - ' + t('failed to load') + ' "'
					+ delta + '" ' + t('block!');
			drupalgap_alert(msg);
		}
		return block;
...
function drupalgap_block_render(region, current_path, block_delta,
	...
	try {
		...
		if (block_settings.access_callback
		...
		if (render_block) {
			...
			if (block) {
				html = module_invoke(block.module, 'block_view', block_delta,
						region);
			}
		}
		return html;
	...
	try {
		...
		if (attributes) {
			$.each(attributes, function(name, value) {
				if (value != '') {
					// @todo - if someone passes in a value with double quotes,
					// this
					// will break. e.g.
...
function drupalgap_check_visibility(type, data) {
	try {
		...
		if (typeof data === 'undefined') {
			console
					.log('drupalgap_check_visibility - WARNING - no data provided for type ('
							+ type + ')');
		}
		// Roles.
		else if (typeof data.roles !== 'undefined' && data.roles
				&& data.roles.value && data.roles.value.length != 0) {
					...
					.each(
							...
							function(page_index, path) {
								...
								if (path == current_path) {
									...
									// It wasn't a direct path match, is there a
									// wildcard that matches
									// the router path?
									if (path.indexOf('*') != -1) {
										var router_path = drupalgap_get_menu_link_router_path(current_path);
										...
										// There's no wildcard in the rule, and
...
/**
 * Given an entity type and entity, this will return the bundle name as a string
 * for the given entity, or null if the bundle is N/A.
 * 
 * @param {String}
 *            entity_type The entity type.
 * @param {Object}
 *            entity The entity JSON object.
 * @return {*}
 */
function drupalgap_get_bundle(entity_type, entity) {
	try {
		...
		default:
			console
					.log('WARNING: drupalgap_get_bundle - unsupported entity type ('
							+ entity_type + ')');
			break;
		}
		return bundle;
...
function drupalgap_get_path(type, name) {
	try {
		...
		if (type == 'module') {
			...
			console.log('WARNING: drupalgap_get_path - unsupported type ('
					+ type + ')');
		}
		return path;
	} catch (error) {
...
/**
 * Given an error message, this will log the message to the console and goto the
 * error page, if it isn't there already. If Drupal.settings.debug is set to
 * true, this function will also alert the error. You may optionally send in a
 * second message that will be displayed to the user via an alert dialog box.
 * 
 * @param {String}
 *            message
 */
function drupalgap_error(message) {
	try {
		...
		var error_message = 'drupalgap_error() - '
				+ arguments.callee.caller.name + ' - ' + message;
		dpm(error_message);
		if (Drupal.settings.debug) {
...
function arg() {
	try {
		...
		if (arguments.length == 0 || arguments.length == 1) {
			...
			// If no arguments were provided just return the split array,
			// otherwise
			...
			// A path was provided, split it into parts, then return the split
			// array
			// if they didn't request a specific index, otherwise return the
			// value of
...
function l() {
	try {
		...
		// Determine if there are any incoming link options, if there are,
		// attach
		// them to the link object. If there are any attributes, extract them
		// from
...
function t(str) {
	...
	if (lang != 'und' && typeof drupalgap.locale[lang] !== 'undefined'
 ...
 * some of the module names don't exist, and are actually implemented inside the
 * field module. If no module is found, it returns false.
 * 
 * @param {String}
 *            type
 * @return {String}
 ...
 * attribute value to be used in the DOM. An optional third argument is a string
 * language code to use. An optional fourth argument is an integer delta value
 * to use on field elements.
 * 
 * @param {String}
 *            name
 * @param {String}
...
function drupalgap_form_get_element_id(name, form_id) {
	try {
		...
		var id = 'edit-' + form_id.toLowerCase().replace(/_/g, '-') + '-'
...
function _drupalgap_form_render_elements(form) {
	try {
		...
		// For each form element, if the element objects name property isn't
		// set,
		// set it, then render the element if access is permitted. While
		// rendering
				...
				.each(
						...
						function(name, element) {
							...
							if (drupalgap_form_element_access(element)) {
								if (element.is_field
										&& element.field_info_instance.widget.weight) {
									content_weighted[element.field_info_instance.widget.weight] = _drupalgap_form_render_element(
											form, element);
								} else {
									// Extract the bundle. Note, on comments the
									// bundle is prefixed with
									// 'comment_node_' so we need to remove that
									// to correctly map to the
									...
									if (form.bundle) {
										...
										if (form.entity_type == 'comment'
												&& form.bundle
														.indexOf('comment_node_') != -1) {
											bundle = form.bundle.replace(
													'comment_node_', '');
										}
									}
									// This is not a field, if it has a weight
									// in
									// field_info_extra_fields use it, otherwise
									// just append it to the
									// content.
									if (form.entity_type
											&& bundle
											&& typeof drupalgap.field_info_extra_fields[bundle][name] !== 'undefined'
											&& typeof drupalgap.field_info_extra_fields[bundle][name].weight !== 'undefined') {
										var weight = drupalgap.field_info_extra_fields[bundle][name].weight;
										content_weighted[weight] = _drupalgap_form_render_element(
												form, element);
									} else {
										content += _drupalgap_form_render_element(
												form, element);
									}
								}
							}
		...
		// Add any form buttons to the form elements html, if access to the
		// button
		// is permitted.
...
function _drupalgap_form_render_element(form, element) {
	try {
		...
		// We'll assume the element has no items (e.g. title, nid, vid, etc),
		// unless
		// we determine later that this element is a field, then it'll have
		// items.
		var items = false;

		// If this element is a field, extract the items from the language code
		// and
		// determine what module and hook will handle the items. If the element
		// is
		// not a field, just flatten it into a single item collection and
		// determine
		// which module handles this element type. Keep in mind not all the
		// modules
		// actually exist, and we've placed implementations into the field
		// module.
		var module = false;
		...
		if (module) {
			...
			if (drupalgap_function_exists(field_widget_form_function_name)) {
				...
				console.log('WARNING: _drupalgap_form_render_element() - '
						+ field_widget_form_function_name
						+ '() does not exist for the "' + element.type
						+ '" form element!');
			}
		}
		...
		$
				.each(
						...
						function(delta, item) {
							...
							// Overwrite the variable's attributes id with the
							// item's id.
							...
							// Create an array for the item's children if it
							// doesn't exist already.
							// This is used by field widget forms to extend form
							// elements.
							if (!items[delta].children) {
								items[delta].children = [];
							}

							// Generate the label for field items on delta zero
							// only. Keep in mind
							// rendered labels, with an element
							// title_placeholder set to true,
							...
							// If the element's title is set to be a
							// placeholder, set the
							// placeholder attribute equal to the title on the
							// current item, unless
							// someone already set it. If it is a required
							// element, mark it as such.
							if (delta == 0
								...
								// @TODO show a better required marker for
								// placeholders.
								/*
								 * if (element.required) { placeholder += ' ' +
								 * theme('form_required_marker', { }); }
								 */
								variables.attributes['placeholder'] = placeholder;
							}

							// If there wasn't a default value provided, set
							// one. Then set the
							// default value into the variables' attributes.
							// Although, if we have an
							...
							// Call the hook_field_widget_form() if necessary.
							// Merge any changes
							// to the item back into this item.
							if (field_widget_form_function) {
								field_widget_form_function.apply(null, [ form,
										null, element.field_info_field,
										element.field_info_instance, language,
										items, delta, element ]);
								// @TODO - sometimes an item gets merged without
								// a type here, why?
							...
							// Merge element attributes into the variables
							// object.
							if (item.options && item.options.attributes) {
								variables.attributes = $.extend(true,
										variables.attributes,
										item.options.attributes);
							}

							// Render the element item, unless it wasn't
							// supported.
							item_html = _drupalgap_form_render_element_item(
									form, element, variables, item);
		...
		/*
		 * if (element.field_info_field && element.field_info_field.cardinality ==
		 * -1) { var add_another_item_variables = { text: 'Add another item',
		 * attributes: { 'class': 'drupalgap_form_add_another_item', onclick:
		 * "javascript:_drupalgap_form_add_another_item('" + form.id + "', '" +
		 * element.name + "', " + delta + ')' } }; html += theme('button',
		 * add_another_item_variables); }
		 */

		// Is this element wrapped? We won't wrap hidden inputs by default,
		// unless
		...
		// Add a label to the element, except submit and hidden elements. Any
		// field
		// labels have already been rendered, other element labels must be
		// manually
		// rendered here. Don't attach the label if the element's
		// title_placeholder
		// is set to true.
		if (element.type != 'submit' && element.type != 'hidden') {
			if (typeof element.title_placeholder !== 'undefined'
					&& element.title_placeholder) { /*
													 * Skip label for
													 * placeholders.
													 */
...
function _drupalgap_form_render_element_item(form, element, variables, item) {
	try {
		var html = '';
		// Depending on the element type, if necessary, adjust the variables
		// and/or
		// theme function to be used, then render the element by calling its
		// theme
		// function.
		...
		if (item.type == 'submit') {
			variables.attributes.onclick = '_drupalgap_form_submit(\''
					+ form.id + '\');';
		...
		// If a value isn't set on variables, try to set it with the default
		// value
		// on the item.
		...
		// Run the item through the theme system if a theme function exists, or
		// try
		// to use the item markup, or let the user know the field isn't
		// supported.
		if (drupalgap_function_exists('theme_' + theme_function)) {
			...
			if (item.markup || item.markup == '') {
				...
				// @todo - the reason for this warning sometimes happens because
				// the
				// item.type is lost with $.extend in
				// _drupalgap_form_render_element().
				// @update - if an item doesn't have a type, it gets set by the
				// parent
				// element, so we should now always have a type available here.
				var msg = 'Field ' + item.type + ' not supported.';
				console.log('WARNING: _drupalgap_form_render_element_item() - '
						+ msg);
				dpm(item);
				return null;
			}
		...
		if (item.children && item.children.length > 0) {
			for ( var i = 0; i < item.children.length; i++) {
				if (item.children[i].markup) {
					...
					console
							.log('WARNING: _drupalgap_form_render_element_item - '
									+ 'failed to render child '
									+ i
									+ ' for '
									+ element.name);
				}
...
/**
 * Given an element name, the form, a language code and a delta value, this will
 * return default values that can be used to place an item element into a Forms
 * API object.
 * 
 * @param {String}
 *            name
...
function _drupalgap_form_element_items_widget_arguments(form, form_state,
	...
	try {
		...
		console
				.log('_drupalgap_form_element_items_widget_arguments - '
						+ error);
	}
}
...
function _drupalgap_form_add_another_item(form_id, name, delta) {
	try {
		...
		// the form, generate default variables for the new item, determine the
		// next
		// delta value.
		...
		var language = language_default();
		var item = drupalgap_form_element_item_create(name, form, language,
				delta + 1);
		...
		var field_widget_form_function = element.field_info_instance.widget.module
				+ '_field_widget_form';
		window[field_widget_form_function].apply(null,
				_drupalgap_form_element_items_widget_arguments(form, null,
						element, language, delta + 1));
		drupalgap_form_local_storage_save(form);
		$(add_another_item_button).before(
				_drupalgap_form_render_element_item(form, element, variables,
						item));
	} catch (error) {
...
/**
 * Given a jQuery selector to a form, this will clear all the elements on the
 * UI.
 * 
...
function drupalgap_form_clear(form_selector) {
	try {
		$(':input', form_selector).not(':button, :submit, :reset, :hidden')
...
function drupalgap_form_defaults(form_id) {
	try {
		...
		form.prefix = '';
		form.suffix = '';
		// Create empty arrays for the form's validation and submission
		// handlers,
		// then add the default call back functions to their respective array,
		// if
		// they exist.
		form.validate = [];
		form.submit = [];
...
function drupalgap_form_render(form) {
	try {
		...
		// think the node edit page gets an id of "node_edit" and possibly so
		// does
		// the node edit form, which also may get an id of "node_edit". We may
		// want
		// to prefix both the template page and form ids with prefixes, e.g.
		// drupalgap_page_* and drupalgap_form_*, but adding these prefixes
		// could
		...
		var form_html = '<form id="' + form.id + '" ' + form_attributes + '>'
				+ prefix + '<div id="drupalgap_form_errors"></div>'
				+ form_elements + suffix + '</form>';
		return form_html;
	} catch (error) {
...
function drupalgap_get_form(form_id) {
	try {
		var html = '';
		var form = drupalgap_form_load.apply(null, Array.prototype.slice
				.call(arguments));
		if (form) {
			// Render the form.
...
function drupalgap_form_load(form_id) {
	try {
		...
		if (drupalgap_function_exists(function_name)) {
			...
			// Build the form arguments by iterating over each argument then
			// adding
			// each to to the form arguments, afterwards remove the argument at
			// index
			...
			// If there were no arguments to pass along, call the function
			// directly to
			...
			if (form_arguments.length == 0) {
				...
				// We must consolidate the form, form_state and arguments into
				// one array
				...
				form = fn.apply(null, Array.prototype.slice
						.call(consolidated_arguments));
			}

			// Set empty options and attributes properties on each form element
			// if the
			// element does not yet have any. This allows others to more easily
			// modify
			// options and attributes on an element without having to worry
			// about
			...
			$
					.each(
							form.elements,
							function(name, element) {
								// If this element is a field, load its
								// field_info_field and
								...
								if (field_info_field) {
									...
									form.elements[name].field_info_instance = drupalgap_field_info_instance(
											form.entity_type, name, form.bundle);
								}
								form.elements[name].is_field = element_is_field;
								// Set the name property on the element if it
								// isn't already set.
								if (!form.elements[name].name) {
									form.elements[name].name = name;
								}
								// If the element is a field, we'll append a
								// language code and delta
								// value to the element id, along with the field
								// items appended
								// onto the element using the language code and
								// delta values.
								var id = null;
								if (element_is_field) {
									// What's the number of allowed values
									// (cardinality) on this field?
									// A cardinality of -1 means the field has
									// unlimited values.
									var cardinality = parseInt(element.field_info_field.cardinality);
									if (cardinality == -1) {
										cardinality = 1; // we'll just add
										// one element for
										// now, until we
										// figure out how to handle the 'add
										// another
										// item' feature.
									}
									// Initialize the item collections language
									// code if it hasn't been.
									...
									for ( var delta = 0; delta < cardinality; delta++) {
										...
										var item = drupalgap_form_element_item_create(
												name, form, language, delta);
										// If the delta for this item hasn't
										// been created on the element,
										// create it using the default item
										// values. Otherwise, merge the
										// default values into the pre existing
										// item on the element.
									...
									// This element is not a field, setup
									// default options if none
									// have been provided. Then set the element
									// id.
									if (!element.options) {
									...
									id = drupalgap_form_get_element_id(name,
											form.id);
			...
			// Place the assembled form into local storage so
			// _drupalgap_form_submit
			// will have access to the assembled form.
			drupalgap_form_local_storage_save(form);
		} else {
			var error_msg = 'drupalgap_form_load - '
					+ t('no callback function') + ' (' + function_name + ') '
					+ t('available for form') + ' (' + form_id + ')';
			drupalgap_alert(error_msg);
		}
		return form;
	} catch (error) {
...
/**
 * Given a form id, this will delete the form from local storage. If the form
 * isn't in local storage, this returns false.
 * 
 * @param {String}
 *            form_id
 * @return {Object}
...
function drupalgap_form_local_storage_delete(form_id) {
	try {
		var result = window.localStorage
				.removeItem(drupalgap_form_id_local_storage_key(form_id));
		return result;
	} catch (error) {
...
/**
 * Given a form id, this will load the form from local storage and return it. If
 * the form isn't in local storage, this returns false.
 * 
 * @param {String}
 *            form_id
 * @return {Object}
...
function drupalgap_form_local_storage_load(form_id) {
	try {
		var form = false;
		form = window.localStorage
				.getItem(drupalgap_form_id_local_storage_key(form_id));
		if (!form) {
			form = false;
		} else {
...
function drupalgap_form_local_storage_save(form) {
	try {
		window.localStorage.setItem(
				drupalgap_form_id_local_storage_key(form.id), JSON
						.stringify(form));
	} catch (error) {
...
/**
 * Given a form id, this will return the local storage key used by DrupalGap to
 * save the assembled form to the device's local storage.
 * 
 * @param {String}
 *            form_id
 * @return {String}
	...
	try {
		...
		var form_validation = function() {
			try {
				...
				$.each(form.validate, function(index, function_name) {
					var fn = window[function_name];
					fn.apply(null, Array.prototype.slice.call([ form,
							form_state ]));
				});
				...
				// If there were validation errors, show the form errors and
				// stop the
				...
				console.log('_drupalgap_form_submit - form_validation - '
						+ error);
			}
		};
		...
		var form_submission = function() {
			try {
				...
				// @todo - we can't do this here because often times a form's
				// submit
				// handler makes asynchronous calls (i.e. user login) and
				// although the
				// form validated, server side may say the input was invalid, so
				// the
				// user will still be on the form, except we already removed the
				// form.
				// drupalgap_form_local_storage_delete(form_id);
			} catch (error) {
				console.log('_drupalgap_form_submit - form_submission - '
						+ error);
			}
		};
		...
		// If this is an entity form, and there is an image field on the form,
		// we
		// need to asynchronously process the image field, then continue onward
		// with normal form validation and submission.
		if (form.entity_type
				&& image_fields_present_on_entity_type(form.entity_type,
						form.bundle)) {
			_image_field_form_process(form, form_state, {
				success : form_validation
			});
		} else {
			// There were no image fields on the form, proceed normally with
			// form
			// validation, which will in turn process the submission if there
			// are no
			// validation errors.
			form_validation();
		}
	...
	try {
		$.each(form.elements, function(name, element) {
			...
			if (element.required) {
				...
				else if (element.type == 'select' && element.required
						&& value == '') {
					valid = false;
				...
				if (!valid) {
					...
					drupalgap_form_set_error(name, t('The') + ' ' + field_title
							+ ' ' + t('field is required') + '.');
				}
			}
		});
 ...
 * each element and value and places them into the form state's values. This is
 * similar to $form_state['values'] in Drupal.
 * 
 * @param {Object}
 *            form
 * @return {Object}
				...
				.each(
						...
						function(name, element) {
							...
							if (element.is_field) {
								...
								if (allowed_values == -1) {
									allowed_values = 1; // Convert unlimited
									// value field to one
									// for now...
								}
								for ( var delta = 0; delta < allowed_values; delta++) {
									id = drupalgap_form_get_element_id(name,
											form.id, lng, delta);
									form_state.values[name][lng][delta] = _drupalgap_form_state_values_assemble_get_element_value(
											id, element);
								}
							} else {
								id = drupalgap_form_get_element_id(name,
										form.id);
								form_state.values[name] = _drupalgap_form_state_values_assemble_get_element_value(
										id, element);
							}
						});
...
function _drupalgap_form_state_values_assemble_get_element_value(id, element) {
	try {
		// If a value_callback is specified on the element, call that function
		// to
		// retrieve the element's value. Ohterwise, we'll use the default
		// techniques
		...
		case 'checkbox':
			...
			break;
		case 'checkboxes':
			// Iterate over each option, and see if it is checked, then pop it
			// onto
			// the value array. It's possible for values to be updated
			// dynamically
			// by a developer (i.e. through a pageshow handler), so it isn't
			// safe to
			// look at the options in local storage. Instead we'll directly
			// iterate
		...
		case 'list_text':
			// Radio buttons.
			if (element.field_info_instance
		...
		console
				.log('_drupalgap_form_state_values_assemble_get_element_value - '
						+ error);
	}
}
		...
		if (typeof responseText === 'object' && responseText.form_errors) {
			...
			$.each(responseText.form_errors, function(element_name, error_msg) {
				if (error_msg != '') {
					...
					// "field_art_type][und", so let's trim anything at and
					// after
					// the first "]".
...
function theme_checkboxes(variables) {
	try {
		...
		$.each(variables.options, function(value, label) {
			...
			html += '<label>' + theme('checkbox', checkbox) + '&nbsp;' + label
					+ '</label>';
		});
		// Check the box?
		/*
		 * if (variables.checked) { variables.attributes.checked = 'checked'; }
		 */
		return html;
	} catch (error) {
		console.log('theme_checkbox - ' + error);
...
function theme_form_element_label(variables) {
	try {
		...
		// By default, use the element id as the label for, unless the element
		// is
		...
		// Render the label.
		var html = '<label for="' + label_for + '"><strong>' + element.title
				+ '</strong>';
		if (element.required) {
	...
	try {
		...
		if (variables.options) {
			...
			$.each(variables.options, function(value, label) {
				...
				var input_label = '<label for="' + input_id + '">' + label
						+ '</label>';
				radios += '<input id="' + input_id + '" value="' + value + '" '
						+ drupalgap_attributes(variables.attributes) + checked
						+ ' />' + input_label;
				delta++;
			});
		}
...
function theme_textarea(variables) {
	try {
		var output = '<div><textarea '
				+ drupalgap_attributes(variables.attributes) + '>'
				+ variables.value + '</textarea></div>';
		return output;
	} catch (error) {
...
function drupalgap_goto(path) {
	try {
		...
		if (!drupalgap.menu_links[router_path]) {
			...
			// If a 404 handler provided a new path use it, otherwise just use
			// the
			// system 404 page. Either way, update the router path before
			// continuing
		...
		// @TODO - for now we're going to skip access checks on local tasks,
		// since
		// they are covered by menu_block_view(), but if someone were to
		// navigate
		// directly to e.g. a node's edit page, they would be able to see the
		// page.
		// Of course Drupal would actually prevent them from updating the node
		// on
		// the live site, but nonetheless this needs to be fixed. It's a tough
		// issue
		// though and related to
		// https://github.com/signalpoint/DrupalGap/issues/257
		if (drupalgap.menu_links[router_path].type != 'MENU_DEFAULT_LOCAL_TASK'
		...
		// If the new router path is the same as the current router path and the
		// new
		...
		if (router_path == drupalgap_router_path_get()
			...
			// If we're reloading the current page, we need to set aside this
			// path
			// and navigate to the system's _reload page, which will then handle
			// the
		...
		// Return if we are trying to go to the path we are already on, unless
		// this
		// was a form submission, then we'll let the page rebuild itself. For
		// accurracy we compare the jQM active page url with the destination
		// page
		// id.
		...
		// @TODO when drupalgap_goto() is called during the bootstrap, we
		// shouldn't
		...
		// page and we're not in the middle of a form submission, prevent the
		// page
		...
		if (drupalgap_page_in_dom(page_id)) {
			...
			// bring
			// them into the current options without overwriting any existing
			// values.
			if (drupalgap.menu_links[router_path].options) {
				options = $.extend({},
						drupalgap.menu_links[router_path].options, options);
			}
			// Reload the page? If so, remove the page from the DOM, delete the
			// reloadPage option, then set the reloadingPage option to true so
			// others
			// down the line will know the page is reloading. We can't pass
			// along the
			// actual reloadPage option since it may collide with jQM later on.
			// We
			// have to use 'force' when removing the page from the DOM since DG
			// won't
			...
			if (typeof options.reloadPage !== 'undefined' && options.reloadPage) {
				// @TODO we may need to leave the query ONLY if the above call
				// to
				// _drupalgap_goto_prepare_path() yielded a query string,
				// otherwise we
				...
				// @TODO changePage has been deprecated as of jQM 1.4.0 and will
				// be
				// removed in 1.50.
		...
		} else if (typeof options.reloadPage !== 'undefined'
				&& options.reloadPage) {
...
/**
 * Generate a JQM page by running it through the theme then attach the page to
 * the <body> of the document, then change to the page. Remember, the rendering
 * of the page does not take place here, that is covered by the pagebeforechange
 * event in theme.inc.js which happens after we change the page here.
 * 
 * @param {String}
 *            path
 * @param {String}
 ...
 *            options
 * @param {Object}
 *            menu_link The menu link object from drupalgap.menu_links.
 */
function drupalgap_goto_generate_page_and_go(path, page_id, options, menu_link) {
	try {
		...
		if (!drupalgap_file_exists(page_template_path)) {
			console.log('drupalgap_goto_generate_page_and_go - '
			...
			// Load the page template html file. Determine if we are going to
			// cache
			...
			if (html) {
				...
				// Let's change to the page. Web apps and the ripple emulator do
				// not
				...
				// Invoke all implementations of
				// hook_drupalgap_goto_post_process().
				module_invoke_all('drupalgap_goto_post_process', path);
			} else {
				drupalgap_alert('drupalgap_goto_generate_page_and_go - '
...
function _drupalgap_goto_prepare_path(path) {
	try {
		...
		// Pull out any query string parameters and populate them into _GET, if
		// we
		...
		if (typeof arguments[1] !== 'undefined' && arguments[1]) {
			...
			if (pos != -1 && pos != path.length - 1) {
				...
				for ( var i = 0; i < parts.length; i++) {
					...
					_GET(decodeURIComponent(query[0]),
							decodeURIComponent(query[1]), path);
				}
			}
		}
		...
		if (path == '') {
			if (!drupalgap.settings.front) {
				drupalgap_alert('drupalgap_goto_prepare_path - '
...
function _drupalgap_back() {
	try {
		// @WARNING - any changes here (except the history.back() call) need to
		// be
		// reflected into the window "navigate" handler below
		drupalgap.back = true;
		history.back();
		drupalgap_path_set(drupalgap.back_path.pop());
		drupalgap_router_path_set(drupalgap_get_menu_link_router_path(drupalgap_path_get()));
	} catch (error) {
		console.log('drupalgap_back' + error);
	}
}
...
$(window)
		.on(
				...
				function(event, data) {
					...
					// In web-app mode, clicking the back button on your browser
					// (or Android
					// device browser), does not actually fire drupalgap_back(),
					// so we mimic
					// it here, but skip the history.back() call because that's
					// already
					// happened via the hardware button.
					if (drupalgap.settings.mode == 'web-app') {
						// Grab the direction we're travelling.
						// back, forward (or undefined, aka moving from splash
						// to front page)
						var direction = data.state.direction;
						if (direction == 'back'
								&& drupalgap.back_path.length > 0) {
							// @WARNING - any changes here should be reflected
							// into
							// _drupalgap_back().
							drupalgap.back = true;
 ...
 * equal to the value, and you may optionally pass in a third argument to use a
 * specific page id, otherwise DrupalGap will automatically use the appropriate
 * page id.
 * 
 * @return {String|NULL}
 */
function _GET() {
	try {
		...
		if (get) {
			...
			// If a page id was provided use it, otherwise use the current
			// page's id.
			var id = null;
			...
			// provided, otherwise return the whole query string object for the
			// page.
		...
		// Set.
			...
			// If we were given a path, use its page id as the property index,
			// other
			...
			// If the id hasn't been instantiated, do so. Then set the key and
			// value
			// onto it.
			if (typeof _dg_GET[id] === 'undefined') {
				_dg_GET[id] = {};
...
$(document)
		.on(
				...
				function(e, data) {
					try {
						// If we're moving backwards, reset drupalgap.back and
						// return.
						if (drupalgap && drupalgap.back) {
							drupalgap.back = false;
							return;
						}
						// If the jqm active page url is the same as the page id
						// of the current
						// path, return.
						if (drupalgap_jqm_active_page_url() == drupalgap_get_page_id(drupalgap_path_get())) {
							return;
						}
						// We only want to process the page we are going to, not
						// the page we are
						// coming from. When data.toPage is a string that is our
						// destination page.
						if (typeof data.toPage === 'string') {

							// If drupalgap_goto() determined that it is
							// necessary to prevent the
							// default page from reloading, then we'll skip the
							// page
...
function template_process_page(variables) {
	try {
		var drupalgap_path = drupalgap_path_get();
		// Execute the active menu handler to assemble the page output. We need
		// to
		// do this before we render the regions below.
		drupalgap.output = menu_execute_active_handler();
		// For each region, render it, then replace the placeholder in the
		// page's
		...
		$.each(drupalgap.theme.regions, function(index, region) {
			page_html = page_html.replace('{:' + region.name + ':}',
...
/**
 * Given a path, this will return the id for the page's div element. For
 * example, a string path of 'foo/bar' would result in an id of 'foo_bar'. If no
 * path is provided, it will return the current page's id.
 * 
 * @param {String}
 *            path
 * @return {String}
...
function drupalgap_add_page_to_dom(options) {
	try {
		...
		attributes = $.extend(true, attributes,
				options.menu_link.options.attributes);
		attributes['class'] += ' '
				+ drupalgap_page_class_get(drupalgap.router_path);
		options.html = options.html.replace(/{:drupalgap_page_attributes:}/g,
...
function drupalgap_remove_page_from_dom(page_id) {
	try {
		...
		if (current_page_id != page_id || options.force) {
			...
			// We'll remove the query string, unless we were instructed to leave
			// it.
			if (typeof _dg_GET[page_id] !== 'undefined'
					&& (typeof options.leaveQuery === 'undefined' || !options.leaveQuery)) {
				delete _dg_GET[page_id];
			}
		} else {
			console
					.log('WARNING: drupalgap_remove_page_from_dom() - not removing '
							+ 'the current page ('
							+ page_id
							+ ') from the DOM!');
		}
 ...
 * 
 * @param {String}
 *            router_path The page router path.
 * @return {String} A css class name.
 */
function drupalgap_page_class_get(router_path) {
	try {
		// Replace '/' and '%' with underscores, then trim any trailing
		// underscores.
...
function drupalgap_jqm_active_page_url() {
	try {
		// WARNING: when the app first loads, this value may be much different
		// than
		// you expect. It certainly is not the front page path, because on
		// Android
		// for example it returns '/android_asset/www/index.html'. Also, when
		// the
...
function drupalgap_render_page() {
	try {
		...
		// based on the output type. The output type will either be an html
		// string
		...
		if (output_type === 'string') {
			...
			if (output.theme && drupalgap.theme_registry[output.theme]) {
				...
				// Extract the theme object template and determine the template
				// file
				// name and path.
				var template = drupalgap.theme_registry[output.theme];
				var template_file_name = output.theme.replace(/_/g, '-')
						+ '.tpl.html';
				var template_file_path = template.path + '/'
						+ template_file_name;
					...
					if (template_file_html) {
						...
						if (placeholders) {
							...
							$.each(placeholders, function(index, placeholder) {
								...
								// even if it was
								// empty.
								template_file_html = template_file_html
										.replace('{:' + placeholder + ':}',
												html);
							});
						} else {
							// There were no place holders found, do nothing,
							// ok.
						}

						// Finally add the rendered template file to the
						// content.
						content += template_file_html;
					} else {
						console
								.log('drupalgap_render_page - failed to get file contents ('
										+ template_file_path + ')');
					}
				} else {
					console
							.log('drupalgap_render_page - template file does not exist ('
									+ template_file_path + ')');
				}
			}

			// Iterate over any remaining variables and theme them.
			// @todo - each remaining variables should have its own container
			// div and
			// unique id, similar to the placeholder div containers mentioned
			// above.
		...
		// Now that we are done assembling the content into an html string, we
		// can
		// return it.
		return content;
	} catch (error) {
...
function drupalgap_render_region(region) {
	try {
		...
		region_html += _drupalgap_region_render_zone('_prefix', region,
				current_path);

		// If the region has blocks specified for it in the theme in
		// settings.js...
		if (drupalgap.settings.blocks[drupalgap.settings.theme][region.name]) {

			// If a class attribute hasn't yet been provided, set a default,
			// then
			// append a system class name for the region onto its attributes
			// array.
			...
			// If there are any links attached to this region, render them
			// first.
			var region_link_count = 0;
			var region_link_popup_count = 0;
			if (region.links && region.links.length > 0) {

				// Let's first iterate over all of the region links and keep
				// counts of
				// any links that use the ui-btn-left and ui-btn-right class
				// attribute.
				// This will allow us to properly wrap region links in a control
				// group.
				...
				// We need to separately render each side of the header (left,
				// right).
				// That allows us to properly wrap the links with a control
				// group if
				// it is needed.
				...
				for ( var i = 0; i < region.links.length; i++) {
					...
					// Check link's region visiblity settings. Links will not be
					// rendered
					// on certain system pages.
					// @TODO - this additional call to
					// drupalgap_check_visibility() here
					// may be expensive, consider setting aside the results from
					// the call
					// above, and using them here.
					if (drupalgap_check_visibility('region', data)) {

						// Don't render the link on certain system pages.
						if (in_array(current_path, [ 'offline', 'error',
								'user/logout' ])) {
							continue;
						}

						// If this is a popup region link, set the jQM
						// attributes to make
						// this link function as a popup (dropdown) menu. Set
						// the default
						...
						if (data.options.popup) {
							...
							// If the link text isn't set, and the data icon pos
							// isn't set,
							// set it the data icon pos so the button and icon
							// are rendered
							// properly.
							...
							// If data-rel, data-icon, data-role aren't set, set
							// them.
							if (typeof data.options.attributes['data-rel'] === 'undefined') {
								data.options.attributes['data-rel'] = 'popup';
							}
							if (typeof data.options.attributes['data-icon'] === 'undefined') {
							...
							// Popup menus need a dynamic href value on the
							// link, so we
							// always overwrite it.
							link_path = null;
							...
							// Set the data-role to a button, if one isn't
							// already set.
							if (typeof data.options.attributes['data-role'] === 'undefined') {
								data.options.attributes['data-role'] = 'button';
							}
						...
						// If it has notext for the icon position, force the
						// text to be
						// an nbsp.
				...
				// If there was more than one link on a side, wrap it in a
				// control
			...
			// Render each block in the region. Determine how many visible
			// blocks are
			// in the region.
			var block_counts = {
				block_count : 0,
			...
			$
					.each(
							...
							function(block_delta, block_settings) {
								...
								if (block_delta == '_prefix'
										|| block_delta == '_suffix') {
									return;
								}

								// Render the block.
								region_html += drupalgap_block_render(region,
										current_path, block_delta,
										block_settings, block_counts);
			...
			if (in_array(region.attributes['data-role'], [ 'header', 'footer' ])
					&& (block_counts.block_count == 0 && region_link_count > 0 || block_counts.block_count
							- block_counts.block_menu_count == 0)
					|| (region_link_count > 0
							&& region_link_popup_count >= block_counts.block_menu_count && block_counts.block_count == 0)) {
				// Show an empty header if we're not collapsing on an empty
				// region.
				if (typeof region.collapse_on_empty === 'undefined'
		...
		region_html += _drupalgap_region_render_zone('_suffix', region,
				current_path);
...
function _drupalgap_region_render_zone(zone, region, current_path) {
	try {
		var html = '';
		var region_settings = drupalgap.settings.blocks[drupalgap.settings.theme][region.name];
		if (typeof region_settings[zone] === 'undefined') {
			return html;
		}
		var blocks = region_settings[zone];
		$.each(blocks, function(block_delta, block_settings) {
			html += drupalgap_block_render(region, current_path, block_delta,
					block_settings);
		});
		return html;
	} catch (error) {
	...
	try {
		...
		if (drupalgap_function_exists(function_name)) {
			...
			if (drupalgap.menu_links[router_path].page_arguments) {
				// For each page argument, if the argument is an integer, grab
				// the
				// corresponding arg(#), otherwise just push the arg onto the
				// page
				// arguments. Then try to prepare any entity that may be present
				// in
				// the url so the entity is sent via the page arguments to the
				// page
				// callback, instead of just sending the integer.
				var args = arg(null, path);
				$.each(drupalgap.menu_links[router_path].page_arguments,
				...
				content = fn.apply(null, Array.prototype.slice
						.call(page_arguments));
			} else {
			...
			// If the content came back as a string, convert it to a render
			// object
			// so any jQM page events can be attached to the content if
			// necessary.
			...
			// event callback functions attached to the menu link for this page,
			// set
			// each event up to be fired with inline JS on the page. We pass
			// along
			...
			for ( var i = 0; i < jqm_page_events.length; i++) {
				if (drupalgap.menu_links[router_path][jqm_page_events[i]]) {
					...
					if (drupalgap_function_exists(jqm_page_event_callback)) {
						...
						console.log('menu_execute_active_handler (' + path
								+ ') - the jQM ' + jqm_page_event
								+ ' call back function '
			...
			console.log('menu_execute_active_handler - no page callback ('
					+ router_path + ')');
...
function menu_router_build() {
	// Calls all hook_menu implementations and builds a collection of menu
	// links.
	try {
		...
		$
				.each(
						modules,
						function(index, module) {
							// Determine the hook function name, grab the
							...
							$
									.each(
											menu_links,
											function(path, menu_item) {
												// Attach module name to item.
												menu_item.module = module;
												// Set a default type for the
												// item if one isn't provided.
												if (typeof menu_item.type === 'undefined') {
													menu_item.type = 'MENU_NORMAL_ITEM';
												}
												// Set default options and
												// attributes if none have been
												// provided.
												// @TODO - Now that we are doing
												// this here, there may be a few
												// places throughout the code
												// that were checking for these
												// undefined objects that will
												...
												// Make the path available as a
												// property in the menu link.
												menu_item.path = path;
												// Determine any parent,
												// sibling, and child paths for
												// the item.
...
function drupalgap_get_menu_link_router_path(path) {
	try {
		...
		// @TODO - Why is this function called twice sometimes? E.G. via an MVC
		// item
		// view item/local_users/user/0, this function gets called twice in one
		// page
		// load, that can't be good.

		// @TODO - this function has a limitation in the types of menu paths it
		// can
		// handle, for example a menu path of 'collection/%/%/list' with a path
		// of
		...
		// record of the result router path, keyed by the incoming path, that
		// way
		// this heavy function can be called more often with less resource.

		// Is this path defined in drupalgap.menu_links? If it is, use it's
		// router
		...
		// If we haven't found a router path yet, try some other techniques to
		// find
		...
		if (!router_path) {
			...
			// candidates as the router for this path? Let's start at the back
			// of the
			// argument list and start replacing each with a wildcard (%) and
			// then
			// see if there is a path in drupalgap.menu_links that can handle
			// it.
	...
	try {
		if (drupalgap.settings.menus) {
			...
			$.each(drupalgap.settings.menus, function(menu_name, menu) {
				// If the menu does not already exist, it is a custom menu, so
				// create
				// the menu and its corresponding block.
				if (!drupalgap.menus[menu_name]) {
					// If the custom menu doesn't have its machine name set, set
					// it.
					if (!menu.menu_name) {
						menu.menu_name = menu_name;
					}
					// Save the custom menu, as long is its name isn't
					// 'regions',
					// because that is a special "system" menu that allows menu
					// links to
					// be placed directly in regions via settings.js. Keep in
					// mind the
					...
					// The menu is a system defined menu, merge it together with
					// any
					// custom settings.
					$.extend(true, drupalgap.menus[menu_name], menu);
				}
			});
			// Now that we have all of the menus loaded up, and the menu router
			// is
			// built, let's iterate over all the menu links and perform various
			// operations on them.
			$
					.each(
							drupalgap.menu_links,
							function(path, menu_link) {
								// Let's grab any links from the router that
								// have a menu specified,
								// and add the link to the router.
								if (menu_link.menu_name) {
									if (drupalgap.menus[menu_link.menu_name]) {
										// Create a links array for the menu if
										// one doesn't exist already.
										if (!drupalgap.menus[menu_link.menu_name].links) {
											drupalgap.menus[menu_link.menu_name].links = [];
										}
										// Add the path to the menu link inside
										// the menu.
										menu_link.path = path;
										// Now push the link onto the menu. We
										// only care about the title,
										// path and options, as this is just a
										// link. The rest of the
										// menu link data can be retrieved from
										// drupalgap.menu_links.
										...
										console
												.log('drupalgap_menus_load - menu does not exist ('
														+ menu_link.menu_name
														+ '), cannot attach link to it ('
														+ path + ')');
									}
								}
								// If the menu link is set to a specific region,
								// create a links array
								// for the region if one doesn't exist already,
			...
			// If there are any region menu links defined in settings.js, create
			// a
			// links array for the region if one doesn't exist already, then add
			// the
			// menu item to the links array as a link.
			if (typeof drupalgap.settings.menus.regions !== 'undefined') {
				$.each(drupalgap.settings.menus.regions,
						function(region, menu) {
							if (typeof menu.links !== 'undefined'
									&& $.isArray(menu.links)
									&& menu.links.length > 0) {
...
function drupalgap_menus_load_convert_menu_link_to_link_json(menu_link) {
	try {
		var link = {};
		if (menu_link.title) {
			// TODO - this is strange, we have to fill the 'text' value so
			// theme_link
			// will play nice. These two properties, and their usage, need a
			// thorough
			// review, only one should probably be used.
			// UPDATE - the link.text property is probably no longer used, it
			// should
			// be safe to get rid of. In fact, this whole function is dumb and
			// should
			// go away.
		...
		console.log('drupalgap_menus_load_convert_menu_link_to_link_json - '
				+ error);
	}
}
	...
	try {
		...
		if (args.length > 1) {
			...
			if (drupalgap.menu_links[parent]) {
				// Now tell the parent about this child. If the parent doesn't
				// yet have
				...
				// Now tell any siblings about this item, and tell this item
				// about any
				// siblings.
...
function theme(hook, variables) {
	try {
		...
		// First see if the current theme implements the hook, if it does use
		// it, if
...
function theme_collapsible(variables) {
	try {
		...
		var html = '<div ' + drupalgap_attributes(variables.attributes) + '>'
				+ '<' + header_type + ' '
				+ drupalgap_attributes(header_attributes) + '>'
				+ variables.header + '</' + header_type + '>'
...
function theme_item(variables) {
	try {
		...
		$.each(variables.item, function(field, value) {
			html += '<h2>' + variables.model.fields[field].title + '</h2>'
					+ '<p>' + value + '</p>';
		});
		return html;
	} catch (error) {
...
function theme_item_list(variables) {
	try {
		// We'll theme an empty list unordered list by default, if there is a
		// type
		...
		html += '<' + type + ' ' + drupalgap_attributes(variables.attributes)
				+ '>';
...
function theme_jqm_item_list(variables) {
	try {
		if (variables.attributes) {
			if (variables.attributes['data-role']
					&& variables.attributes['data-role'] != 'listview') {
			} else {
				variables.attributes['data-role'] = 'listview';
			}
...
function theme_link(variables) {
	try {
		...
		if (typeof variables.path !== 'undefined' && variables.path != null) {
			...
			// If the path begins with a hashtag, just render the link as is
			// with the
			...
			// By default our onclick will use a drupalgap_goto(). If we have
			// any
			...
			if (variables.options) {
				...
				if (variables.options.InAppBrowser) {
					onclick = "window.open('" + variables.path
							+ "', '_blank', 'location=yes');";
				}
					...
					// All other options need to be extracted into a JSON string
					// for the
					// onclick handler.
					...
					onclick = 'drupalgap_goto(\'' + variables.path + '\', '
							+ '{' + goto_options + '});';
			...
			return '<a href="#" onclick="javascript:' + onclick + '"'
					+ drupalgap_attributes(variables.attributes) + '>' + text
					+ '</a>';
...
function theme_logout(variables) {
	try {
		return bl(t('Logout'), 'user/logout', {
			attributes : {
				'data-icon' : 'action',
				'data-iconpos' : 'right'
			}
...
function theme_submit(variables) {
	try {
		return '<button ' + drupalgap_attributes(variables.attributes) + '>'
				+ variables.element.value + '</button>';
	} catch (error) {
	...
	try {
		...
		if (typeof drupalgap.menu_links[router_path].title_callback !== 'undefined') {
			...
			if (drupalgap_function_exists(function_name)) {
				...
				var fn = window[function_name];
				// Place the internal success callback handler on the front of
				// the
				// title arguments.
				title_arguments.unshift(_drupalgap_page_title_pageshow_success);
				// Are there any additional arguments to send to the title
				// callback?
				if (drupalgap.menu_links[router_path].title_arguments) {
					// For each title argument, if the argument is an integer,
					// grab the
					// corresponding arg(#), otherwise just push the arg onto
					// the title
					// arguments.
					var args = arg(null, drupalgap_path_get());
					$.each(drupalgap.menu_links[router_path].title_arguments,
							function(index, object) {
								if (is_int(object) && args[object]) {
				...
				drupalgap_set_title(fn.apply(null, Array.prototype.slice
						.call(title_arguments)));
			}
		} else {
...
function comment_page_view(cid) {
	try {
		if (cid) {
			var content = {
				container : _drupalgap_entity_page_container('comment', cid,
						'view')
			};
			return content;
		} else {
	...
	try {
		comment_load(cid, {
			success : function(comment) {
				...
				_drupalgap_entity_page_container_inject('comment', cid, 'view',
						content);
			}
		});
	} catch (error) {
...
function comment_page_title(callback, cid) {
	try {
		// Try to load the comment subject, then send it back to the given
		// callback.
		var title = '';
...
function comment_edit(form, form_state, comment, node) {
	try {
		...
		// If there is no comment object coming in, make an empty one with a
		// node
		// id. Note, once the form.js submit handler is aware of its own entity
		// and
		// can pass it along to this function (and others) when loading a form
		// during the submission process, this little chunk of code will no
		// longer
		// be needed.
		if (!comment) {
			comment = {
		...
		drupalgap_entity_add_core_fields_to_form('comment', bundle, form,
				comment);

		// Add the fields for this content type to the form.
		drupalgap_field_info_instances_add_to_form('comment', bundle, form,
				comment);
...
function comment_services_postprocess(options, result) {
	try {
		if (options.service == 'comment' && options.resource == 'create') {
			// If we're on the node view page, inject the comment into the
			// comment
				...
				node_load(nid, {
					...
					success : function(node) {
						comment_load(result.cid, {
							success : function(comment) {
								$(container).append(theme('comment', {
									comment : comment
								})).trigger('create');
								scrollToElement('#' + container_id
										+ ' :last-child', 500);
								var form_selector = '#'
...
function theme_comments(variables) {
	try {
		...
		variables.attributes['class'] += 'comments comments-node-'
				+ variables.node.type;
		variables.attributes['data-role'] = 'collapsible-set';
		// Open the container.
		var html = '<div ' + drupalgap_attributes(variables.attributes) + '>';
...
function theme_comment(variables) {
	try {
		...
		// Append comment extra fields and content. The user info will be
		// rendered
		// as a list item link.
		var author = picture + '<h3>' + comment.name + '</h3>' + '<p>'
				+ created + '</p>';
		author = l(author, 'user/' + comment.uid);
		comment_content += '<h2>' + comment.subject + '</h2>'
				+ '<ul data-role="listview" data-inset="true">' + '<li>'
				+ author + '</li>' + '</ul>' + comment.content;
		...
		contact_index({
			success : function(results) {
				...
				$.each(results, function(index, result) {
					...
					var option = '<option value="' + result.cid + '" '
	...
	contact_site({
		...
		success : function(result) {
			if (result[0]) {
				...
				drupalgap_alert(t('There was a problem sending your message!'),
						{
							title : t('Error')
						});
			}
...
function contact_personal_form(form, form_state, recipient) {
	try {
		// @TODO - when providing a personal contact form, make sure the user
		// has
 ...
 *            entity
 */
function drupalgap_entity_add_core_fields_to_form(entity_type, bundle, form,
		entity) {
	try {
		...
		// there is a value present in the entity, then set the field's form
		// element
...
function drupalgap_entity_assemble_data(entity_type, bundle, entity, options) {
	try {
		console
				.log('WARNING: drupalgap_entity_assemble_data() has been '
...
function drupalgap_entity_edit_form_delete_confirmation(entity_type, entity_id) {
	try {
		var confirm_msg = t('Delete this content, are you sure? This action cannot be undone...');
		drupalgap_confirm(
				confirm_msg,
				{
					confirmCallback : function(button) {
						...
						call_arguments.success = function(result) {
							// Remove the entities page from the DOM, if it
							// exists.
							var entity_page_path = entity_type + '/'
									+ entity_id;
							...
							window.localStorage
									.removeItem(entity_local_storage_key(
											entity_type, entity_id));
							// Go to the front page, unless a form action path
							// was specified.
		...
		console
				.log('drupalgap_entity_edit_form_delete_confirmation - '
						+ error);
	}
}
...
function drupalgap_entity_render_content(entity_type, entity) {
	try {
		entity.content = '';
		// Render each field on the entity, using the default display. The
		// fields
		// need to be appended according to their weight, so we'll keep track of
		// the weights and displays, then at the end we'll render them and
		// append
		...
		$
				.each(
						...
						function(field_name, field) {
							// Determine which display mode to use. The default
							// mode will be used
							...
							if (field.display['drupalgap']) {
								...
								// If a module isn't listed on the drupalgap
								// display, use the default
								// display's module.
								if (typeof display.module === 'undefined'
		...
		// For each sorted weight, locate the field with the corresponding
		// weight,
		...
		module_invoke_all('entity_post_render_content', entity, entity_type,
				bundle);
		// Update this entity in local storage so the content property sticks.
		_entity_local_storage_save(entity_type,
				entity[entity_primary_key(entity_type)], entity);
	} catch (error) {
...
function drupalgap_entity_render_field(entity_type, entity, field_name, field,
	...
	try {
		...
		// then determine the hook's function name, then render the field
		// content.
		...
		if (!module) {
			if (!field.widget.module) {
				var msg = 'drupalgap_entity_render_field - '
						+ 'unable to locate the module for the field ('
						+ field_name + ')';
				console.log(msg);
				return content;
			} else {
		...
		if (drupalgap_function_exists(function_name)) {
			...
			// from the entity, then call the formatter function and append its
			// result
			// to the entity's content.
			var fn = window[function_name];
			var items = null;
			// Check to see if translated content based on app's language
			// setting
			...
			// language code on the entity, but still have 'und' on the field,
			// so
			...
			var elements = fn(entity_type, entity, field, /*
															 * This is actually
															 * the instance,
															 * doh! (I think)
															 */
			drupalgap_field_info_field(field_name), language, items, display);
			...
			console.log('WARNING: drupalgap_entity_render_field - '
					+ function_name + '() ' + 'does not exist! (' + field_name
					+ ')');
		}
		...
		// Finally, wrap the rendered field in a div, and set the field name as
		// the
		...
		module_invoke_all('entity_post_render_field', entity, field_name,
				field, reference);
		...
		console.log('drupalgap_entity_render_field - ' + error);
	}
}
		...
		$
				.each(
						...
						function(name, value) {
							...
							if (typeof form.elements[name].access !== 'undefined'
									&& !form.elements[name].access) {
								return;
							}

							// Determine wether or not this element is a field.
							// If it is, determine
							...
							// Retrieve the potential key for the element, if we
							// don't get one
							// then it is a flat field that should be attached
							// as a property to the
							// entity. Otherwise attach the key and value to the
							// entity.
							var key = drupalgap_field_key(name); // e.g.
							// value,
							// fid, tid,
							// nid, etc.
							if (key) {

								// Determine how many allowed values for this
								// field.
								var allowed_values = form.elements[name].field_info_field.cardinality;

								// Convert unlimited value fields to one, for
								// now...
								if (allowed_values == -1) {
									allowed_values = 1;
								}

								// Make sure there is at least one value before
								// creating the form
								...
								// Create an empty object to house the field on
								// the entity.
								entity[name] = {};

								// Some fields do not use a delta value in the
								// service call, so we
								// prepare for that here.
								// @todo - Do all options_select widgets really
								// have no delta value?
								// Or is it only single value fields that don't
								// have it? We need to
								// test this.
								var use_delta = true;
								if (form.elements[name].type == 'taxonomy_term_reference'
										|| form.elements[name].field_info_instance.widget.type == 'options_select') {
									use_delta = false;
								...
								// Now iterate over each delta on the form
								// element, and add the value
								// to the entity.
								for ( var delta = 0; delta < allowed_values; delta++) {
									if (typeof value[language][delta] !== 'undefined') {

										// @TODO - the way values are determined
										// here is turning into
										// spaghetti code. Every form element
										// needs its own
										// value_callback, just like Drupal's
										// FAPI. Right now DG has
										// something similar going on with the
										// use of
										// hook_assemble_form_state_into_field().
										// So replace any spaghetti
										// below with a value_callback. Provide
										// a deprecated hook warning
										// for any fields not haven't caught up
										// yet, and fallback to the
										// hook for a while.
										// @UPDATE - Actually, the DG FAPI
										// hook_assemble_form_state_into_field()
										// is a good idea, and
										// should be used by all field form
										// elements, then in
										// drupalgap_field_info_instances_add_to_form(),
										// that function
										// should use the value_callback idea to
										// properly map entity data
										// to the form element's value.

										// Extract the value.
										var field_value = value[language][delta];

										// By default, we'll assume we'll be
										// attaching this element item's
										// value according to a key (usually
										// 'value' is the default key
										// used by Drupal fields). However,
										// we'll give modules that
										// implement
										// hook_assemble_form_state_into_field()
										// an opportunity
										// to specify no usage of a key if their
										// item doesn't need one.
										// The geofield module is an example of
										// field that doesn't use a
										// key. The use_wrapper flag allows
										// others to completely override
										// the use of a wrapper around the field
										// value, e.g. taxonomy term
										// reference autocomplete. We'll attach
										// any other helpful
										// variables here as well (element name,
										// form id, etc).
										var field_key = {
											value : 'value',
											use_key : true,
										...
										// If this element is a field, give the
										// field's module an
										// opportunity to assemble its own
										// value, otherwise we'll just
										...
										// If we don't need a delta value, place
										// the field value using the
										// key, if posible. If we're using a
										// delta value, push the key
										// and value onto the field to indicate
										// the delta.
										if (!field_key.use_delta) {
											if (!field_key.use_wrapper) {
												...
												if ($
														.isArray(entity[name][language])) {
													console
															.log('WARNING: drupalgap_entity_build_from_form_state - '
																	+ 'cannot use key ('
																	+ key
																	+ ') on field ('
																	+ name
										...
										// If the field value was null, we won't
										// send along the field, so
										// just remove it. Except for
										// list_boolean fields, they need a
										// null value to set the field value to
										// false.
										// @TODO - will this cause issues with
										// multi value fields? i.e. if
										// delta zero is null, but delta one
										// isn't, this will probably
										...
										// If we had an optional select list,
										// and no options were
										// selected, delete the empty field from
										// the assembled entity.
										// @TODO - will this cause multi value
										// issues?
										if (is_field
												&& !use_delta
												&& form.elements[name].field_info_instance.widget.type == 'options_select'
	...
	try {
		...
		call_arguments.success = function(result) {
			try {
				...
				// by default we'll try to redirect to [entity-type]/[entity-id]
				// to view
				// the entity. For taxonomy, we replace the underscore with a
				// forward
				// slash in the path.
				var destination = form.action;
				if (!destination) {
					var prefix = form.entity_type;
					if (prefix == 'taxonomy_vocabulary'
							|| prefix == 'taxonomy_term') {
						prefix = prefix.replace('_', '/');
					}
					destination = prefix + '/' + result[primary_key];
				}
				// Is there a destination URL query parameter overwriting the
				// action?
				if (_GET('destination')) {
					destination = _GET('destination');
				}
				// Set up the default goto options, and use any options provided
				// by the
				// form.
				var goto_options = {
					form_submission : true
				...
				if (form.action_options) {
					goto_options = $.extend({}, goto_options,
							form.action_options);
				}
				...
				console
						.log('drupalgap_entity_form_submit - success - '
								+ error);
			}
		};
		...
		call_arguments.error = function(xhr, status, message) {
			try {
				...
				var msg = _drupalgap_form_submit_response_errors(form,
						form_state, xhr, status, message);
				if (msg) {
					drupalgap_alert(msg);
				}
			} catch (error) {
		...
		// Depending on if we are creating a new entity, or editing an existing
		// one,
		...
		if (editing) {
			...
			// Remove the entity from local storage.
			// @todo This should be moved to jDrupal.
			window.localStorage.removeItem(entity_local_storage_key(
					form.entity_type, entity[primary_key]));
		}
		var fn = window[services_get_resource_function_for_entity(
				form.entity_type, crud)];
...
function drupalgap_entity_get_core_fields(entity_type, bundle) {
	try {
		...
		// @todo - And why is this function not populated dynamically via
		// Drupal?
		var fields = {};
		switch (entity_type) {
		case 'comment':
			...
			// Only anonymous users can fill out the name field, authenticated
			// users
			...
			var comment_anonymous = drupalgap.content_types_list[content_type].comment_anonymous;
		...
		default:
			console
					.log('drupalgap_entity_get_core_fields - entity type not supported yet ('
							+ entity_type + ')');
			break;
		}
		return fields;
 ...
 *             Given an entity type, this returns the primary key identifier for
 *             it.
 * @param {String}
 *            entity_type
 * @return {String}
...
function drupalgap_entity_get_primary_key(entity_type) {
	try {
		console
				.log('WARNING: drupalgap_entity_get_primary_key() is deprecated! '
...
function _drupalgap_entity_page_container(entity_type, entity_id, mode) {
	try {
		var id = _drupalgap_entity_page_container_id(entity_type, entity_id,
				mode);
		var attrs = {
			id : id,
...
function _drupalgap_entity_page_container_inject(entity_type, entity_id, mode,
		build) {
	try {
		// Get the container id, set the drupalgap.output to the page build,
		// then
...
function entity_page_edit(form_id, entity_type, entity_id) {
	try {
		var content = {
			container : _drupalgap_entity_page_container(entity_type,
					entity_id, 'edit')
		};
		return content;
	} catch (error) {
	...
	try {
		entity_load(entity_type, entity_id, {
			success : function(entity) {
				_drupalgap_entity_page_container_inject(entity_type, entity_id,
						'edit', drupalgap_get_form(form_id, entity));
			}
		});
	} catch (error) {
...
function entity_primary_key_title(entity_type) {
	try {
		...
		case 'user':
			...
			break;
		default:
			console.log('entity_primary_key_title - unsupported entity type ('
					+ entity_type + ')');
			break;
		}
		return key;
...
function entity_services_request_pre_postprocess_alter(options, result) {
	try {
		// If we're retrieving an entity, render the entity's content, if it
		// isn't
		// already set.
		...
		// If we're indexing comments, render its content, if it isn't already
		// set.
			...
			$.each(result,
					function(index, object) {
						...
						drupalgap_entity_render_content(options.service,
								result[index]);
					});
		}
	} catch (error) {
	...
	try {
		...
		if (!instances[field_name]) {
			var msg = 'WARNING: drupalgap_field_info_instance - ' + '"'
					+ field_name + '" does not exist for entity ('
					+ entity_type + ')' + ' bundle (' + bundle_name + ')';
			console.log(msg);
			return null;
		}
...
function drupalgap_field_info_instances(entity_type, bundle_name) {
	try {
		...
		// @TODO there appears to be a special case with commerce_products, in
		// that
		// they aren't wrapped like normal entities (see the else statement when
		// a
		// bundle name isn't present). Or do we have a bug here, and we
		// shouldn't
		...
		if (!bundle_name) {
			if (entity_type == 'commerce_product') {
				...
				field_info_instances = drupalgap.field_info_instances[entity_type][entity_type];
			}
		} else {
...
/**
 * Given an entity type, bundle, form and entity, this will add the entity's
 * fields to the given form.
 * 
 * @param {String}
 *            entity_type
 * @param {String}
 ...
 *            entity
 */
function drupalgap_field_info_instances_add_to_form(entity_type, bundle, form,
		entity) {
	try {
		...
		// Iterate over each field in the entity and add it to the form. If
		// there is
		// a value present in the entity, then set the field's form element
		// default
		...
		if (fields) {
			$
					.each(
							fields,
							function(name, field) {
								// The user registration form is a special case,
								// in that we only want
								// to place fields that are set to display on
								...
								if (field_info) {
									...
									if (entity && entity[name]
										...
										for ( var delta = 0; delta < cardinality; delta++) {
											// @TODO - is this where we need to
											// use the idea of the
											// value_callback property present
											// in Drupal's FAPI? That way
											// each element knows how to map the
											// entity data to its element
											// value property.
											...
											// If the default_value is null, set
											// it to an empty string.
											if (default_value == null) {
												default_value = '';
											}
											// @todo - It appears not all fields
											// have a language code to use
											// here, for example taxonomy term
											// reference fields don't!
											form.elements[name][language][delta] = {
												value : default_value
											};
											// Place the field item onto the
											// element.
											if (entity[name][language][delta]) {
												form.elements[name][language][delta].item = entity[name][language][delta];
											}
										}
									}

									// Give module's a chance to alter their own
									// element during the form
									// build, that way element properties will
									// be saved to local storage
									// and then available during
									// hook_field_widget_form() and the form
									// submission process.
									var fn = field.widget.module
											+ '_field_info_instance_add_to_form';
									if (drupalgap_function_exists(fn)) {
										window[fn](entity_type, bundle, form,
												entity, form.elements[name]);
									}
...
/**
 * Given a field name, this will return the key that should be used when setting
 * its value on an entity. If the field name is not a field, it returns false.
 * 
 * @param {String}
 *            field_name
 * @return {String}
	...
	try {
		...
		if (field_info) {
			...
			if (field_info.module == 'image' && field_info.type == 'image') {
				key = 'fid';
			} else if (field_info.module == 'taxonomy'
		...
		if (!empty(items)) {
			$.each(items, function(delta, item) {
				...
				if (display.type == 'list_default') {
					...
					// Single on/off checkboxes need an empty space as their
					// markup so
					// just the label gets rendered.
					if (instance.type == 'list_boolean'
...
function list_assemble_form_state_into_field(entity_type, bundle,
	...
	try {
		...
		switch (field.type) {
		case 'list_boolean':
			// For single on/off checkboxes, if the checkbox is unchecked, then
			// we
			// send a null value on the language code. We know the checkbox is
			// unchecked if the form_state_value is equal to the first allowed
			// value
			// on the field.
			if (instance.widget.type == 'options_onoff') {
				...
				console
						.log('WARNING: list_assemble_form_state_into_field - unknown widget ('
								+ field.type + ') on list_boolean');
			}
			break;
		case 'list_text':
			// For radio buttons on the user entity form, field values must be
			// "flattened", i.e. this field_foo: { und: [ { value: 123 }]},
			// should
		...
		default:
			console
					.log('WARNING: list_assemble_form_state_into_field - unknown type ('
							+ field.type + ')');
			break;
		}
		return result;
...
function list_views_exposed_filter(form, form_state, element, filter, field) {
	try {
		...
		if (widget == 'select') {
			...
			// Set the options, then depending on whether or not it is required,
			// set
			...
			dpm('WARNING: list_views_exposed_filter - unsupported widget ('
					+ widget + ')');
		}
	} catch (error) {
	...
	try {
		...
		case 'list_boolean':
			...
			case 'options_onoff':
				...
				if ($.isArray(field.settings.allowed_values)) {
					...
					$.each(field.settings.allowed_values,
				...
				if (typeof items[delta] !== 'undefined'
						&& items[delta].value == on) {
					items[delta].options.attributes['checked'] = 'checked';
				}
				break;
			default:
				console
						.log('WARNING: options_field_widget_form list_boolean with '
								+ 'unsupported type ('
		...
		case 'list_integer':
			if (instance) {
				...
				case 'options_select':
					...
					// If the select list is required, add a 'Select' option and
					// set
					// it as the default. If it is optional, place a "none"
					// option
					...
					// If more than one value is allowed, turn it into a
					// multiple
					// select list.
				...
				default:
					console
							.log('WARNING: options_field_widget_form - unsupported widget ('
									+ instance.widget.type + ')');
					return false;
					break;
				}
				// If there are any allowed values, place them on the options
				// list. Then check for a default value, and set it if
				// necessary.
				if (field && field.settings.allowed_values) {
					$.each(field.settings.allowed_values, function(key, value) {
						...
						// If the value already exists in the options, then
						// someone
						// else has populated the list (e.g. commerce), so don't
						// do
						// any processing.
		...
		case 'taxonomy_term_reference':
			...
			if (instance.widget.type == 'options_select') {
				...
				console.log('WARNING: options_field_widget_form() - '
						+ instance.widget.type + ' not yet supported for '
...
/**
 * Implements hook_field_formatter_view(). /** Implements
...
function image_field_widget_form(form, form_state, field, instance, langcode,
	...
	try {
		...
		// If we're dealing with the user profile 'picture' it isn't a real
		// field,
		...
		if (typeof items[delta].item !== 'undefined' && items[delta].item.fid) {
			...
			// Show the image on the form, the file name as a link to the actual
			// file,
			...
			/*
			 * theme('button_link', { text: 'Remove', path: null, attributes: {
			 * onclick: "_image_field_widget_form_remove_image()", 'data-icon':
			 * 'delete', 'data-iconpos': 'right' } });
			 */
			// '<div class="filesize">(' + items[delta].item.filesize +
			// ')</div>';
		...
		var imagefield_destination_type = item_id_base
				+ '_imagefield_destination_type';
		...
		// div, an image item, a button to add an image, and a button to browse
		// for
		// images.
		...
		var html = '<div>'
				...
				+ '<a href="#" data-role="button" data-icon="camera" data-iconpos="notext"'
				+ 'id="'
				+ items[delta].id
				+ '-button">'
				+ button_text
				+ '</a>'
				+ '<a href="#" data-role="button" data-icon="grid" data-iconpos="notext"'
				+ 'id="' + browse_button_id + '">' + browse_button_text
				+ '</a>' + '</div>';
		...
		html += '$("#' + drupalgap_get_page_id(drupalgap_path_get())
				+ '").on("pageshow",function(){' + 'document.addEventListener('
				+ '"deviceready", ' + event_listener + ', ' + 'false);' + '});'
				+ 'function ' + event_listener + '() {' + image_field_source
				+ ' = navigator.camera.PictureSourceType;'
				+ imagefield_destination_type
		...
		html += 'function ' + imagefield_error + '(message) {'
				...
				+ 'message != "no image selected")' + '{' + 'console.log("'
				+ imagefield_error + '");' + 'drupalgap_alert(message);' + '}'
				+ '}';
		...
		html += '$("#' + items[delta].id + '-button").on("click",function(){'
				+ 'var photo_options = {' + 'quality: ' + quality + ','
				+ 'destinationType: ' + imagefield_destination_type
				+ '.DATA_URL,' + 'correctOrientation: true' + '};'
				+ 'navigator.camera.getPicture(' + imagefield_success + ', '
				+ imagefield_error + ', ' + 'photo_options);' + '});';
		// Add click handler for browse button.
		html += '$("#' + browse_button_id + '").on("click",function(){'
				+ 'var browse_photo_options = {' + 'quality: ' + quality + ','
				+ 'sourceType: ' + image_field_source + '.PHOTOLIBRARY,'
				+ 'destinationType: ' + imagefield_destination_type
				+ '.DATA_URL,' + 'correctOrientation: true' + '};'
				+ 'navigator.camera.getPicture(' + imagefield_success + ', '
				+ imagefield_error + ', ' + 'browse_photo_options);' + '});';
...
function image_style_url(style_name, path) {
	try {
		var src = Drupal.settings.site_path + Drupal.settings.base_path + path;
		if (src.indexOf('public://') != -1) {
			src = src.replace('public://', Drupal.settings.file_public_path
					+ '/styles/' + style_name + '/public/');
		} else if (src.indexOf('private://') != -1) {
			src = src.replace('private://', Drupal.settings.file_private_path
					+ '/styles/' + style_name + '/private/');
		}
		return src;
	} catch (error) {
...
function _image_phonegap_camera_getPicture_success(options) {
	try {
		...
		// Show the captured photo as a thumbnail. When the photo is loaded,
		// resize
...
function _image_field_form_process(form, form_state, options) {
	try {
		...
		$.each(form.image_fields, function(index, name) {
			...
			// Build the data for the file create resource. If it's private,
			// adjust
			// the filepath.
			var file = {
				file : {
			...
			file_save(file, {
				...
				success : function(result) {
					try {
						// Set the hidden input and form state values with the
						// file id.
						var element_id = drupalgap_form_get_element_id(name,
								form.id);
						...
						console.log('_image_field_form_process - success - '
								+ error);
					}
				}
			});
...
function menu_block_view(delta, region) {
	...
	try {
		...
		// Since menu link paths may have an 'access_callback' handler that
		// needs
		// to make an async call to the server (e.g. local tasks), we'll utilize
		// a
		...
		// If the menu is wrapped, check to see if any wrap_options attributes
		// were
		// attached. If any were provided use them to build the div container
		// attributes. We will always overwrite the container id though, since
		// it is
		// generated by the system (and used to dynamically inject the menu
		// html).
		var container_attributes = {};
		if (typeof menu.options !== 'undefined'
...
function menu_block_view_pageshow(options) {
	try {
		...
		// Grab current path so we can watch out for any menu links that match
		// it.
		var path = drupalgap_path_get();
		...
		if (delta == 'primary_local_tasks') {
			...
			// For the current page's router path, grab any local task menu
			// links add
			// them into the menu. Note, local tasks are located in a menu link
			// item's
			// children, or its parent's children (including itself), if there
			// are
			// any. Local tasks typically have argument wildcards in them, so
			// we'll
			...
			if (drupalgap.menu_links[router_path]) {
				...
				} else if (parent && drupalgap.menu_links[parent]
						&& drupalgap.menu_links[parent].children) {
					...
					try {
						...
						if (local_tasks && !empty(local_tasks)) {
							$
									.each(
											local_tasks,
											function(index, local_task) {
												if (drupalgap.menu_links[local_task]
														&& (drupalgap.menu_links[local_task].type == 'MENU_DEFAULT_LOCAL_TASK' || drupalgap.menu_links[local_task].type == 'MENU_LOCAL_TASK')) {
													if (drupalgap_menu_access(
															local_task, null,
															result)) {
														menu_items
																.push(drupalgap.menu_links[local_task]);
													}
												}
											});
						}
						// If there was only one local task menu item, and it is
						// the default
						// local task, don't render the menu, otherwise render
						// the menu as
						...
						if (menu_items.length == 1
							...
							$
									.each(
											menu_items,
											function(index, item) {
												// Make a deep copy of the menu
												// link so we don't modify it.
												var link = jQuery.extend(true,
														{}, item);
												// If there are no link options,
												// set up defaults.
												if (!link.options) {
													link.options = {
														attributes : {}
												...
												// If the link points to the
												// current path, set it as
												// active.
												// We first need to figure out
												// which path to check, by
												// default
												// use the link path, but if its
												// a default local task, use its
												// parent path.
												...
												if (path_to_check == router_path) {
													...
													link.options.attributes['class'] += ' ui-btn ui-btn-active ui-state-persist ';
												}
												items
														.push(l(
																link.title,
						...
						// Inject the html.
						$('#' + options.container_id).html(html).trigger(
								'create');
						// If the block's region is a jQM navbar, refresh the
						// navbar.
						if (options['data-role']
								&& options['data-role'] == 'navbar') {
						...
						console.log('menu_block_view_pageshow - success - '
								+ error);
					}
				};

				// First, determine if any child has an entity arg in the path,
				// and/or
					...
					$
							.each(
									...
									function(index, local_task) {
										if (drupalgap.menu_links[local_task]
												&& (drupalgap.menu_links[local_task].type == 'MENU_DEFAULT_LOCAL_TASK' || drupalgap.menu_links[local_task].type == 'MENU_LOCAL_TASK')) {
											if (drupalgap_path_has_entity_arg(arg(
													null, local_task))) {
												has_entity_arg = true;
											}
											if (typeof drupalgap.menu_links[local_task].access_callback !== 'undefined') {
				...
				// If we have an entity arg, and an access_callback, let's load
				// up the
				...
				if (has_entity_arg && has_access_callback) {
					...
					// Determine the naming convention for the entity load
					// function.
					...
					if (drupalgap_function_exists(load_function)) {
						...
						// Load the entity. MVC items need to pass along the
						// module name and
						// model type to its load function. All other entity
						// load functions
						...
						console
								.log('menu_block_view_pageshow - load function not '
			...
			// If the block's corresponding menu exists, and it has links,
			// iterate
			// over each link, add it to an items array, then theme an item
			// list.
			var menu = false;
			if (drupalgap.menus[delta] && drupalgap.menus[delta].links) {
				...
				$
						.each(
								menu.links,
								function(index, menu_link) {
									// Make a deep copy of the menu link so we
									// don't modify it.
									var link = jQuery.extend(true, {},
											menu_link);
									// If there are no link options, set up
									// defaults.
									if (!link.options) {
										link.options = {
											attributes : {}
									...
									// If the link points to the current path,
									// set it as active.
									if (link.path == path) {
										if (!link.options.attributes['class']) {
											link.options.attributes['class'] = '';
									...
									items.push(l(t(link.title), link.path,
											link.options));
								});
				if (items.length > 0) {
			...
			var wrap = false;
			if (menu && typeof menu.options !== 'undefined'
					&& typeof menu.options.wrap !== 'undefined'
					&& menu.options.wrap) {
				wrap = true;
			}
...
/**
 * Given a menu, this adds it to drupalgap.menus. See menu_list_system_menus for
 * examples of a menu JSON object.
 * 
 * @param {Object}
 *            menu
 */
...
/**
 * Given a module name and model type, this will return the collection JSON
 * object.
 * 
 * @param {String}
 *            module
...
function collection_load(module, type) {
	try {
		return JSON.parse(window.localStorage.getItem(mvc_get_collection_key(
				'collection', module, type)));
	} catch (error) {
...
function collection_save(module, type, collection) {
	try {
		window.localStorage.setItem(mvc_get_collection_key('collection',
				module, type), JSON.stringify(collection));
	} catch (error) {
...
function mvc_install() {
	try {
		// Load models...
		// For each module that implements hook_mvc_model(), iterate over each
		// model
		...
		for ( var i = 0; i < modules.length; i++) {
			...
			if (models) {
				...
				$.each(models, function(model_type, model) {
					...
					// on the model fields. These are the
					// mvc_model_system_fields().
					model.fields.id = {
						'type' : 'hidden',
						'title' : t('ID'),
						'required' : false
					...
					// Add each model type to its namespace within
					// drupalgap.mvc.models
					drupalgap.mvc.models[module][model_type] = model;
					// Save an empty collection to local storage for this model
					// type, if
					// one doesn't already exist.
					var collection_key = mvc_get_collection_key('collection',
							module, model_type);
					if (!window.localStorage.getItem(collection_key)) {
						window.localStorage.setItem(collection_key, '[]');
						// Save settings for the collection to local storage.
						// The auto
						// increment value represents an item id, we start
						// counting at
						// zero since the collection is an array.
						window.localStorage.setItem(mvc_get_collection_key(
								'settings', module, model_type),
								'{"auto_increment":0}');
					}
				});
			}
		...
		// These may not be needed. Perhaps we should just call assumed hooks
		// that
		// should be implemented for any custom views and controllers at the
		// time
		// they are needed, probably no need to bundle them inside
		// drupalgap.mvc.
...
function model_load(module, name) {
	try {
		if (drupalgap.mvc.models[module] === 'undefined'
...
function item_create_form(form, form_state, module, type) {
	try {
		...
		if (model) {
			// @todo - this could be dangerous just overriding the elements
			// variable,
...
function item_create_form_submit(form, form_state) {
	try {
		...
		if (item_save(form_state.values)) {
			/*
			 * var path = 'item/' + form_state.values.module + '/' +
			 * form_state.values.type + '/' + form_state.values.id;
			 */
			var path = 'mvc/collection/list/' + form_state.values.module + '/'
...
function item_save(item) {
	try {
		...
		var settings_key = mvc_get_collection_key('settings', item.module,
				item.type);
...
function node_access(node) {
	try {
		if ((node.uid == Drupal.user.uid && user_access('edit own ' + node.type
				+ ' content'))
...
function node_add_page() {
	try {
		...
		var items = [];
		$.each(Drupal.user.content_types_user_permissions, function(type,
				permissions) {
...
function node_edit(form, form_state, node) {
	try {
		...
		// Add the fields for this content type to the form.
		drupalgap_field_info_instances_add_to_form('node', node.type, form,
				node);
		...
		views_datasource_get_view_result(
				'drupalgap/views_datasource/drupalgap_content', {
					success : function(content) {
						// Extract the nodes into items, then drop them in the
						// list.
						var items = [];
						$.each(content.nodes, function(index, object) {
							items.push(l(object.node.title, 'node/'
									+ object.node.nid));
						});
						drupalgap_item_list_populate('#node_listing_items',
								items);
					}
				});
	} catch (error) {
...
function node_page_view(nid) {
	try {
		if (nid) {
			var content = {
				container : _drupalgap_entity_page_container('node', nid,
						'view')
			};
			return content;
		} else {
		...
		node_load(
				nid,
				{
					success : function(node) {
						...
						// By this point the node's content has been assembled
						// into an html
						// string. This is because when a node is retrieved from
						// the server,
						// we use a services post processor to render its
						// content.
						// @see entity_services_request_pre_postprocess_alter()

						// Does anyone want to take over the rendering of this
						// content type?
						// Any implementors of hook_node_page_view_alter_TYPE()?
						// @TODO this should probably be moved up to the entity
						// level.
						...
						if (modules.length > 0) {
							if (modules.length > 1) {
								var msg = 'node_page_view_pageshow - WARNING - there is more '
										+ 'than one module implementing hook_'
										+ hook
										+ '(), we will '
						...
						if (node.title_field
								&& node.title_field[default_language]) {
						...
						var build = {
							...
							'node' : node,
							// @todo - this is a core field and should by
							// fetched from entity.js
							'title' : {
								'markup' : node_title
							},
							'content' : {
						...
						if (typeof node.comment === 'undefined') {
							_drupalgap_entity_page_container_inject('node',
									node.nid, 'view', build);
						}
						// If the comments are closed (1) or open (2), show the
						// comments.
						else if (node.comment != 0) {
							if (node.comment == 1 || node.comment == 2) {
								// Render the comment form, so we can add it to
								// the content later.
								var comment_form = '';
								if (node.comment == 2) {
								...
								if (node.comment_count != 0) {
									...
									// There weren't any comments, append an
									// empty comments wrapper
									// and show the comment form if comments are
									// open, then inject
									// the page.
									if (node.comment == 2) {
										build.content.markup += theme(
							...
							// Comments are hidden (0), append an empty comments
							// wrapper to the
...
function search_block_form(form, form_state) {
	try {
		...
		form.options.attributes['onsubmit'] = "_drupalgap_form_submit('"
				+ form.id + "'); return false;";
		return form;
	} catch (error) {
 ...
 * 
 * @param {String}
 *            keys The keyword(s) to search for.
 * @param {Object}
 *            options
 */
function search_node(keys, options) {
	try {
		options.method = 'GET';
		options.path = 'search_node/retrieve.json&keys='
...
function theme_search_result_node(variables) {
	try {
		return l('<h2>' + variables.title + '</h2>' + '<p>' + variables.snippet
				+ '</p>', 'node/' + variables.node.nid);
	} catch (error) {
...
function drupalgap_services_get_entity_resource(entity_type) {
	try {
		console
				.log('WARNING: drupalgap_services_get_entity_resource() is deprecated! '
						+ 'Use services_get_resource_function_for_entity() instead.');
		// @todo - deprecate this function, it is no longer needed now that
		// entity
...
function drupalgap_service_resource_extract_results(options) {
	try {
		if (options.service == 'system' && options.resource == 'connect') {
			...
			// options.data.user to overwrite Drupal.user, so anything we want
			// in
			// the Drupal.user object must be added to options.data.user
			// isntead.
...
function system_block_view(delta) {
	try {
		...
		case 'main':
			...
			// region for the content of a page to show up (nodes, users,
			// taxonomy,
			// comments, etc). Depending on the menu link router, we need to
			// route
			// this through the appropriate template files and functions.
			return drupalgap_render_page();
			break;
		case 'messages':
			// If there are any messages waiting to be displayed, render them,
			// then
			...
			$.each(drupalgap.messages, function(index, msg) {
				html += '<div class="messages ' + msg.type + '">' + msg.message
						+ '</div>';
			});
			drupalgap.messages = [];
			return html;
		...
		case 'logo':
			if (drupalgap.settings.logo) {
				return '<div class="logo">' + l(theme('image', {
		...
		case 'title':
			...
			break;
		case 'powered_by':
			return '<p style="text-align: center;">' + t('Realizado por')
					+ ': ' + l('ideasOrange', 'http://www.ideasorange.com', {
						InAppBrowser : true
					}) + '</p>';
			break;
		case 'help':
	...
	try {
		...
		if (_system_reload_messages && !empty(_system_reload_messages)) {
			for ( var i = 0; i < _system_reload_messages.length; i++) {
				drupalgap_set_message(_system_reload_messages[i].message,
...
function system_dashboard_page() {
	try {
		...
		// Verificamos si esta logueado el usuario
		if (Drupal.user.uid != 0) {
			// redirigimos el usuario a la galeria
			drupalgap_goto('galeria_de_usuarios');
			// return;
		}
		...
		content.welcome = {
			markup : '<h2 style="text-align: center;">'
					+ t('Fiesta de Disfraces 2015') + '</h2>'
					+ '<p style="text-align: center;">'
					+ t('#SeLoQueQuierasSer') + '</p>'
		};
		if (drupalgap.settings.logo) {
			content.logo = {
				markup : '<center>' + theme('image', {
					path : drupalgap.settings.logo
				}) + '</center>'
			};
		}
		if (Drupal.user.uid == 0) {
			content.login = {
				theme : 'button_link',
				text : t('Entrar con tu Correo'),
				path : 'user/register'
			/*
			 * attributes:{ onclick: 'login_correo();' }, options:{
			 * reloadPage:true }
			 */
			};
			...
			content.logout = {
				...
				// path: 'login();',
				attributes : {
					onclick : 'logout_correo();'
				}
			};
		}
		content.entradas = {
			theme : 'button_link',
			text : t('Comprar Entradas'),
			path : 'http://www.fiestadedisfraces.com.ar/2014/',
			options : {
				InAppBrowser : true
			}
		};
...
function system_error_page() {
	var content = {
		info : {
			markup : '<p>' + t('Ha ocurrido un error inesperado!') + '</p>'
		}
	};
	return content;
}
...
function system_offline_page() {
	try {
		var content = {
			'message' : {
				'markup' : '<h2>' + t('Failed Connection') + '</h2>' + '<p>'
						+ t("Oops! We couldn't connect to") + ':</p>' + '<p>'
						+ Drupal.settings.site_path + '</p>'
			},
			'try_again' : {
				'theme' : 'button',
				'text' : t('Try Again'),
...
function system_logout_block_access_callback(options) {
	try {
		...
		if (args && args.length == 2 && args[0] == 'user'
 ...
 * 
 * @param {String}
 *            string The permission, such as "administer nodes", being checked
 *            for.
 * @return {Boolean}
 */
...
function user_edit_access(account) {
	try {
		// If the current user is looking at their own account, or if they have
		// the
		// 'administer users' permission, then they are allowed to edit the
		// account.
	...
	try {
		...
		views_datasource_get_view_result(
				'drupalgap/views_datasource/drupalgap_users', {
					success : function(data) {
						// Extract the users into items, then drop them in the
						// list.
						var items = [];
						$.each(data.users, function(index, object) {
							items.push(l(object.user.name, 'user/'
									+ object.user.uid));
						});
						drupalgap_item_list_populate('#user_listing_items',
								items);
					}
				});
	} catch (error) {
...
/**
 * The user logout pageshow callback. This actually handles the call to the user
 * logout service resource.
 */
...
function user_view(uid) {
	try {
		if (uid) {
			var content = {
				container : _drupalgap_entity_page_container('user', uid,
						'view')
			};
			return content;
		} else {
...
function user_view_pageshow(uid) {
	try {
		user_load(uid, {
			success : function(account) {
				// Determine the incoming arguments, and set defaults if
				// necessary.
				var view_mode = 'full';
				var langcode = null;
				if (arguments[1]) {
				...
				if (account) {
					var build = {
						...
						'created' : {
							markup : '<div class="user_profile_history"><h3>'
									+ t('History')
									+ '</h3>'
									+ '<dl><dt>'
									+ t('Member since')
					...
					_drupalgap_entity_page_container_inject('user',
							account.uid, 'view', build);
				}
			}
		});
	} catch (error) {
...
function user_register_form_validate(form, form_state) {
	try {
		// If e-mail verification is not required, make sure the passwords
		// match.
		if (!drupalgap.site_settings.user_email_verification
				&& form_state.values.pass != form_state.values.pass2) {
			drupalgap_form_set_error('pass', t('Passwords do not match!'));
		}
		// If there are two e-mail address fields on the form, make sure they
		// match.
		if (!empty(form_state.values.mail)
				&& !empty(form_state.values.conf_mail)
				&& form_state.values.mail != form_state.values.conf_mail) {
			drupalgap_form_set_error('mail',
					t('E-mail addresses do not match!'));
		}
	} catch (error) {
		...
		user_register(
				account,
				{
					success : function(data) {
						...
						if (!drupalgap.site_settings.user_email_verification) {
							// E-mail verification not needed, if administrator
							// approval is
							...
							if (drupalgap.site_settings.user_register == '2') {
								...
								drupalgap_alert(
										...
										options);
								// If we're automatically logging in do it,
...
function user_profile_form(form, form_state, account) {
	try {
		...
		// If profile pictures are disabled, remove the core field from the
		// form.
		if (drupalgap.site_settings.user_pictures == 0) {
			delete form.elements.picture;
		}

		// Add password fields to the form. We show the current password field
		// only
		// if the user is editing their account. We show the password and
		// confirm
...
function user_profile_form_validate(form, form_state) {
	try {
		// If they entered their current password, and entered new passwords,
		// make
		...
		if (!empty(form_state.values['current_pass'])) {
			if (!empty(form_state.values['pass_pass1'])
				...
				drupalgap_form_set_error('pass_pass1',
						t('Passwords do not match.'));
			}
		}
		// If they didn't enter their current password and entered new
		// passwords,
...
function user_profile_form_submit(form, form_state) {
	try {
		var account = drupalgap_entity_build_from_form_state(form, form_state);
		// If they provided their current password, and their new password,
		// prepare
		// the account submission values.
		if (account.current_pass && !empty(account.pass_pass1)
...
function taxonomy_field_formatter_view(entity_type, entity, field, instance,
	...
	try {
		...
		// It's possible the term items are wrapped in a language code, if they
		// are,
		// pull them out.
 ...
 *            element
 */
function taxonomy_field_widget_form(form, form_state, field, instance,
	...
	try {
		...
		var js = '<script type="text/javascript">' + '$("#' + list_id
				+ '").on("filterablebeforefilter",' + 'function(e, d) {'
 ...
 * 
 * @param {String}
 *            id The id of the hidden input that will hold the term id.
 * @param {Number}
 *            vid
 * @param {Object}
 *            list The unordered list that displays the terms.
 * @param {Object}
 *            e
 * @param {Object}
 *            data
...
function _taxonomy_field_widget_form_autocomplete(id, vid, list, e, data) {
	try {
		...
		$(_taxonomy_field_widget_form_autocomplete_input).on('change',
				function() {
					$('#' + id).val($(this).val());
				});
		...
		if (value && value.length > 0) {
			...
			taxonomy_term_index(query, {
				success : function(terms) {
					if (terms.length != 0) {
						// Extract the terms into items, then drop them in the
						// list.
						var items = [];
 ...
 * 
 * @param {String}
 *            id The id of the hidden input that will hold the term name.
 * @param {String}
 *            list_id The id of the list that holds the terms.
 * @param {Object}
 *            item The list item that was just clicked.
 */
...
function taxonomy_form_vocabulary(form, form_state, vocabulary) {
	try {
		...
		// Add the entity's core fields to the form.
		drupalgap_entity_add_core_fields_to_form('taxonomy_vocabulary', null,
				form, vocabulary);
		...
		if (vocabulary && vocabulary.vid && user_access('administer taxonomy')) {
			form.buttons['delete'] = drupalgap_entity_edit_form_delete_button(
					'taxonomy_vocabulary', vocabulary.vid);
		}
...
function taxonomy_form_vocabulary_submit(form, form_state) {
	try {
		var vocabulary = drupalgap_entity_build_from_form_state(form,
				form_state);
...
function taxonomy_form_term(form, form_state, term) {
	try {
		...
		// Add the entity's core fields to the form.
		drupalgap_entity_add_core_fields_to_form('taxonomy_term', null, form,
				term);
...
function taxonomy_term_page(tid) {
	try {
		if (tid) {
			var content = {
				container : _drupalgap_entity_page_container('taxonomy_term',
						tid, 'view'),
	...
	try {
		taxonomy_term_load(tid, {
			success : function(term) {
				...
				_drupalgap_entity_page_container_inject('taxonomy_term',
						term.tid, 'view', content);
				taxonomy_term_selectNodes(term.tid,
						{
							success : function(results) {
								// Extract the nodes into items, then drop them
								// in the list.
								var items = [];
								$.each(results, function(index, node) {
									items
											.push(l(node.title, 'node/'
													+ node.nid));
								});
								drupalgap_item_list_populate(
		...
		taxonomy_vocabulary_index(null,
				{
					success : function(vocabularies) {
						// Extract the vocabs into items, then drop them in the
						// list.
						var items = [];
						$.each(vocabularies, function(index, vocabulary) {
							items.push(l(vocabulary.name,
									'taxonomy/vocabulary/' + vocabulary.vid));
						});
						drupalgap_item_list_populate(
								'#vocabulary_listing_items', items);
					}
				});
	} catch (error) {
...
function taxonomy_vocabulary_page(vid) {
	try {
		if (vid) {
			...
			console
					.log('taxonomy_vocabulary_page - No vocabulary id provided!');
		}
	} catch (error) {
	...
	try {
		taxonomy_vocabulary_load(vid, {
			success : function(vocabulary) {
				...
				_drupalgap_entity_page_container_inject('taxonomy_vocabulary',
						vocabulary.vid, 'view', content);
				var query = {
					parameters : {
						vid : vid
					}
				...
				taxonomy_term_index(query, {
					success : function(terms) {
						if (terms.length != 0) {
							// Extract the terms into items, then drop them in
							// the list.
							var items = [];
							$.each(terms, function(index, term) {
								items.push(l(term.name, 'taxonomy/term/'
										+ term.tid));
							});
							drupalgap_item_list_populate(
...
/**
 * Given a vocabulary machine name this will return the JSON object for the
 * vocabulary that is attached to the drupalgap.taxonomy_vocabularies object, or
...
function theme_taxonomy_term_reference(variables) {
	try {
		...
		// Is anyone flagging this as required? i.e. is it a views exposed
		// filter?
		// @TODO - the field system that assembles the elements onto a form,
		// when it
		// is a taxonomy term reference field, we need to pass along the
		// required
		// option so when we load the items into the select list, we know
		// whether or
		// not to include the "empty string" option. This is probably in
		// field.js.
		var required = false;
		...
		if (drupalgap_function_exists(widget_function)) {
			...
			// If the options were previously set aside for this widget, use
			// them.
			var options_available = false;
			if (_taxonomy_term_reference_terms[variables.attributes.id]) {
				options_available = true;
				widget_variables.options = _taxonomy_term_reference_terms[variables.attributes.id];
			}

			// Was their a value present to include as the default value for the
			// widget, if so include it. If not, and this filter is not
			// required, set
			// the default value to an empty string so the widget renders the
			// default
			...
			// If the options weren't available, attach a pageshow handler to
			// the
			// current page that will load the terms into the widget. Keep in
			// mind,
			// this inline pageshow handler only gets called once when the first
			// view
			// is loaded. That's why we later set aside the options so they can
			// be
			...
			console.log('WARNING: theme_taxonomy_term_reference() - '
		...
		taxonomy_term_index(
				query,
				{
					success : function(terms) {
						...
						// As we iterate over the terms, we'll set them aside in
						// a JSON
						...
						// If it's not required, place an empty option on the
						// widget and set
						// it aside.
						if (!options.required) {
							var option = null;
							if (options.exposed) {
								option = '<option value="All">- ' + t('Any')
										+ ' -</option>';
								_taxonomy_term_reference_terms[options.element_id]['All'] = '- Any -';
							} else {
								option = '<option value="">- ' + t('None')
										+ ' -</option>';
								_taxonomy_term_reference_terms[options.element_id][''] = '- None -';
							}
							$(widget).append(option);
						}

						// Place each term in the widget as an option, and set
						// the option
						// aside.
						$
								.each(
...
/**
 * An internal function used by a taxonomy term reference field widget to detect
 * changes on it and populate the hidden field that holds the tid in the form.
 * 
 * @param {Object}
 *            input
 * @param {String}
...
function taxonomy_views_exposed_filter(form, form_state, element, filter, field) {
	try {
		/*
		 * dpm('taxonomy_views_exposed_filter'); dpm(element); dpm(filter);
		 * dpm(field);
		 */

		// @TODO this filter loses its value after one submission, aka the next
		// submission will submit it as 'All' eventhough we have a term selected
		// in
		// the widget from the previous submission.
		// Change the input to hidden, then iterate over each vocabulary and
		// inject
		// them into the widget. We'll just use a taxonomy term reference field
		// and
		// fake its instance.
		element.type = 'hidden';
		$.each(field.settings.allowed_values, function(index, object) {
			...
			// Add the widget as a child to the form element, including a label
			// if
...
function views_datasource_get_view_result(path, options) {
	try {
		// Since we do not use clean URLs, replace any potential question marks
		// from
		...
		// If local storage caching is enabled, let's see if we can load the
		// results
		// from there. If we successfully loaded the result, make sure it didn't
		// expire. If it did expire, remove it from local storage. If we don't
		// have
		// it in local storage, or local storage caching is disabled, call
		// Drupal to
		...
		if (options.reset) {
			...
			if (result) {
				...
				if (typeof result.expiration !== 'undefined'
						&& result.expiration != 0 && time() > result.expiration) {
			...
			success : function(result) {
				try {
					if (options.success) {
						...
						result.path = path;
						// If any views caching is enabled, cache the results in
						// local
						// storage.
						if (Drupal.settings.cache.views.enabled) {
							var expiration = time()
									+ Drupal.settings.cache.views.expiration;
					...
					console.log('views_datasource_get_view_result - success - '
							+ error);
				}
			},
			error : function(xhr, status, message) {
...
function views_exposed_form(form, form_state, options) {
	try {
		...
		// @TODO we tried to make the filters collapsible, but jQM doesn't seem
		// to
		...
		$
				.each(
						options.filter,
						function(views_field, filter) {

							// Prep the element basics.
							var element_id = null;
							var element = null;
							// @TODO - This ID is NOT unique enough, it will
							// cause DOM collisions if
							// the same exposed filter gets used twice...
							// @TODO - we should be attaching the value right
							// here, so filter
							// implementors don't need to extract it on their
							// own. Once this is
							// implemented. Then update *_views_exposed_filter()
							// to use this value
							// instead.
							...
							element = {
								...
								options : {
									attributes : {
										id : drupalgap_form_get_element_id(
												element_id, form.id)
									}
								},
							...
							// if (!empty(filter.value)) { element.value =
							// filter.value[0]; }

							// Grab the field name and figure out which module
							// is in charge of it.
							...
							if (field_name) {
								...
								// Grab the field info, and determine the module
								// that will handle it.
								// Then see if hook_views_exposed_filter() has
								// been implemented by
								// that module. That module will be used to
								// assemble the element. If
								// we don't have a handler then just skip the
								// filter.
								...
								if (!drupalgap_function_exists(handler)) {
									dpm('WARNING: views_exposed_form() - '
											+ handler + '() must be '
											+ 'created to assemble the '
											+ field.type + ' filter used by '
											+ field_name);
									return;
								}

								// We have a handler, let's call it so the
								// element can be assembled.
								window[handler](form, form_state, element,
										filter, field);

							} else {
								// This is NOT an entity field, so it is
								// probably a core field (e.g.
								// nid, status, etc). Let's assemble the
								// element. In some cases we may
								// just be able to forward it to a pre-existing
								// handler.
								if (element.type == 'select') {
									list_views_exposed_filter(form, form_state,
											element, filter, null);
								} else {
									dpm('WARNING: views_exposed_form() - I do not know how to handle '
											+ 'the exposed filter for the "'
											+ views_field + '" field');
									dpm(filter);
									return;
								}
...
function views_exposed_form_submit(form, form_state) {
	try {
		...
		// If there is a query set aside from previous requests, and it is equal
		// to
		// the submitted query, then stop the submission. Otherwise remove it
		// from
		// the path.
		if (_views_exposed_filter_query) {
			...
			if (form.variables.path.indexOf('&' + _views_exposed_filter_query) != -1) {
				form.variables.path = form.variables.path.replace('&'
						+ _views_exposed_filter_query, '');
			}
		}

		// Set aside a copy of the query string, so it can be removed from the
		// path
		// upon subsequent submissions of the form.
		_views_exposed_filter_query = query;

		// Indicate that we have an exposed filter, so the reset button can
		// easily
		// be shown/hidden.
		_views_exposed_filter_reset = true;

		// Update the path for the view, reset the pager, hold onto the
		// variables,
		// then theme the view.
...
function views_exposed_form_reset() {
	try {
		// Reset the path to the view, the page, and the global vars, then
		// re-theme
		// the view.
		_views_exposed_filter_submit_variables.path = _views_exposed_filter_submit_variables.path
				.replace('&' + _views_exposed_filter_query, '');
...
function theme_view(variables) {
	try {
		...
		if (!variables.attributes.id) {
			console
					.log('WARNING: theme_view() - No id specified on attributes! '
							+ 'A random id will be generated instead.');
			variables.attributes.id = 'views-view--' + user_password();
		}
		// Since multiple pages can stack up in the DOM, warn the developer if
		// they
		// re-use a Views ID that is already in the DOM. If the ID hasn't been
		// used
		...
		if (in_array(variables.attributes.id, drupalgap.views.ids)) {
			...
			// really in the DOM, remove the id from drupalgap.views.ids and
			// continue
			// onward.
			if (!$('#' + variables.attributes.id)) {
				// @see http://stackoverflow.com/a/3596141/763010
				drupalgap.views.ids.splice($.inArray(variables.attributes.id,
						drupalgap.views.ids), 1);
			} else {
				dpm('WARNING: theme_view() - this id already exists in the DOM: #'
						+ variables.attributes.id
		...
		// Since we'll by making an asynchronous call to load the view, we'll
		// just
		// return an empty div container, with a script snippet to load the
		// view.
...
function _theme_view(variables) {
	try {
		...
		// Make a copy of variables and prepare the success callback for
		// embedding
		// the view.
...
function theme_views_view(variables) {
	try {
		...
		if (variables.title) {
			var title_attributes = variables.title_attributes ? variables.title_attributes
					: null;
			html += theme('header', {
				text : variables.title,
				attributes : title_attributes
			});
			// Place spacers after the header for each format, except
			// unformatted.
		...
		if (typeof results.view.exposed_data !== 'undefined') {
			views_exposed_form_html = drupalgap_get_form('views_exposed_form',
					{
		...
		// Depending on the format, let's render the container opening and
		// closing,
		...
		if (typeof variables.format_attributes !== 'undefined') {
			format_attributes = $.extend(true, format_attributes,
		...
		case 'jqm_table':
			if (variables.format == 'jqm_table') {
				...
				console
						.log('WARNING: theme_views_view() - jqm_table not supported, yet');
			}
		...
		$.each(results[root], function(count, object) {
			...
			var row_content = '';
			if (variables.row_callback
					&& function_exists(variables.row_callback)) {
		...
		// Append the rendered rows and the pager to the html string according
		// to
		// the pager position.
		if (pager_pos == 'top') {
			html += pager;
		...
		// Since the views content is injected dynamically after the page is
		// loaded,
...
function theme_pager(variables) {
	try {
		...
		if (items.length > 0) {
			// Make sure we have an id to use since we need to dynamically build
			// the
			// navbar container for the pager. If we don't have one, generate a
			// random
			// one.
			var id = 'theme_pager_' + user_password();
			html += '<div id="' + id + '" data-role="navbar">'
					+ theme('item_list', {
						items : items
...
function _theme_pager_link_onclick(variables) {
	try {
		...
		// will be available in the _views_embed_view_results variable if they
		// are
		// needed
...
function login_correo() {
	user_login("ideas", "Frank_1984", {
		success : function(result) {
			alert('Aaaaadentro !!!, ' + result.user.name + '!');
		},
		error : function(xhr, status, message) {
			alert(message);
		}
...
function logout_correo() {
	user_logout({
		success : function(result) {
			if (result[0]) {
				alert("Pa juiiiiiiraa !!!");
			}
		}
	});
}
...
// openFB.init({appId: '474315632721064'});
// Uncomment the line below to store the Facebook token in localStorage instead
// of sessionStorage
openFB.init({
	appId : '474315632721064',
...
function login() {
	openFB.login(function(response) {
		if (response.status === 'connected') {
			...
			alert('Error al conectar con Facebook: ' + response.error);
		}
	}, {
		scope : 'email,read_stream,publish_actions'
	});
}
// IO: Funcion que loguea al usuario en la app y si no lo encuentra como usuario
// de
// Drupal le crea uno usando los datos recuperados de Facebook.
function getInfoForLogin() {
	openFB.api({
		path : '/me',
		success : function(data) {
			console.log(JSON.stringify(data));
			// Buscamos al usuario en Drupal
			user_login(data.name, data.id, {
				success : function(result) {
					// drupalgap_alert('Hola '+ result.user.name, {
					// alertCallback: function() {
					drupalgap_goto('galeria_de_usuarios');
					// drupalgap_goto('primera_vez_usuario');
					// }
					// });
				},
				error : function(xhr, status, message) {
					// alert(message);
					// Se crea un nuevo usuario a traves de los datos de
					// Facebook
					var account = {
						name : data.name,
						mail : data.email,
						pass : data.id
					// imagen:'http://graph.facebook.com/' + data.id +
					// '/picture'
					};
					user_save(account, {
						success : function(result) {
							// drupalgap_toast('<p>Ya sos parte de La Previa
							// Paraná !</p>');

							// logueamos al usuario recientemente creado
							user_login(data.name, data.id, {
								success : function(result) {
									drupalgap_goto('galeria_de_usuarios');
								},
	...
	openFB
			.api({
				...
				success : function(data) {
					...
					document.getElementById("userPic").src = 'http://graph.facebook.com/'
							+ data.id + '/picture?type=small';
				},
				error : errorHandler
			});
}
...
function comentar() {
	var comment = {
		...
		comment_body : {
			und : [ {
				value : 'En que van a la FDD 2015?'
			} ]
		}
	};
